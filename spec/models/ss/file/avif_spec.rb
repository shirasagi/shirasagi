...
describe SS::File, dbscope: :example do
  ...
  it do
    subject.thumb.tap do |thumb|
      ...
      expect(thumb.url).to eq "/fs/#{subject.id}/_/#{::File.basename(basename, ".*")}_thumb.avif"
      expect(thumb.full_url).to be_nil
      expect(thumb.name).to eq "#{::File.basename(basename, ".*")}_thumb.avif"
      expect(thumb.filename).to eq "#{::File.basename(basename, ".*")}_thumb.avif"
      expect(thumb.download_filename).to eq "#{::File.basename(basename, ".*")}_thumb.avif"
      expect(thumb.content_type).to eq "image/avif"
