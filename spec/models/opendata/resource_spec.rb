...
describe Opendata::Resource, dbscope: :example do
  ...
  context "when japanese filename is uploaded" do
    ...
    its(:url) { is_expected.to end_with("/#{SS::FilenameConvertor.convert("index - コピー.html")}") }
    its(:full_url) { is_expected.to end_with("/#{SS::FilenameConvertor.convert("index - コピー.html")}") }
  end
end
