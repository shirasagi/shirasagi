...
describe Opendata::App, dbscope: :example do
  ...
  let!(:tag3) { unique_id }
  let!(:app1) do
    create(:opendata_app, cur_site: site, cur_node: node, text: "#{keyword1} #{keyword2}", tags: [ tag1, tag2 ])
  end
  let!(:app2) do
    create(:opendata_app, cur_site: site, cur_node: node, text: "#{keyword2} #{keyword3}", tags: [ tag2, tag3 ])
  end
  let!(:app3) do
    create(:opendata_app, cur_site: site, cur_node: node, text: "#{keyword3} #{keyword1}", tags: [ tag1, tag3 ])
  end
  let!(:app_closed) do
    create(
