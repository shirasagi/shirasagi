...
describe Opendata::Dataset, dbscope: :example do
  let(:site) { cms_site }
  let!(:node_category) { create(:opendata_node_category, cur_site: site) }
  ...
  let!(:tag3) { unique_id }
  let!(:dataset1) do
    create(:opendata_dataset, cur_site: site, cur_node: node, text: "#{keyword1} #{keyword2}", tags: [ tag1, tag2 ])
  end
  let!(:dataset2) do
    create(:opendata_dataset, cur_site: site, cur_node: node, text: "#{keyword2} #{keyword3}", tags: [ tag2, tag3 ])
  end
  let!(:dataset3) do
    create(:opendata_dataset, cur_site: site, cur_node: node, text: "#{keyword3} #{keyword1}", tags: [ tag1, tag3 ])
  end
  let!(:dataset_closed) do
    create(
      :opendata_dataset, cur_site: site, cur_node: node, state: "closed",
  ...
  let!(:dataset_other) do
    create(
      :opendata_dataset, cur_site: site_other, cur_node: node_other,
      text: "#{keyword1} #{keyword2} #{keyword3}", tags: [ tag1, tag2, tag3 ]
    )
  end
