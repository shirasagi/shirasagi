...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/faq.csv" }
  ...
  context "faq nodes" do
    it "#import" do
      # Check initial node count
      expect(Faq::Node::Base.count).to eq 0
      ...
      expect(Faq::Node::Base.count).to eq(csv.size)

      Faq::Node::Base.each_with_index do |node, index|
