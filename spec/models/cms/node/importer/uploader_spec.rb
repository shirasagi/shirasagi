...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/uploader.csv" }
  ...
  context "uploader nodes" do
    it "#import" do
      # Check initial node count
      expect(Uploader::Node::Base.count).to eq 0
      ...
      expect(Uploader::Node::Base.count).to eq(csv.size)

      Uploader::Node::Base.each_with_index do |node, index|
