...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/ads.csv" }
  ...
  context "ads nodes" do
    it "#import" do
      # Check initial node count
      expect(Ads::Node::Banner.count).to eq 0
      ...
      expect(Ads::Node::Banner.count).to eq(csv.size)

      Ads::Node::Banner.each_with_index do |node, index|
