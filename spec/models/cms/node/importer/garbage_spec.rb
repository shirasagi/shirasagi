...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/garbage.csv" }
  ...
  context "garbage nodes" do
    it "#import" do
      # Check initial node count
      expect(Garbage::Node::Base.count).to eq 0
      ...
      expect(Garbage::Node::Base.count).to eq(csv.size)

      Garbage::Node::Base.each_with_index do |node, index|
