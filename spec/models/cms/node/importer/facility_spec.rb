...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/facility.csv" }
  ...
  context "facility nodes" do
    it "#import" do
      # Check initial node count
      expect(Facility::Node::Base.count).to eq 0
      ...
      expect(Facility::Node::Base.count).to eq(csv.size)

      Facility::Node::Base.each_with_index do |node, index|
