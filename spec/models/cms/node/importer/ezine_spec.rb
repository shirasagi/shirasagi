...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/ezine.csv" }
  ...
  context "ezine nodes" do
    it "#import" do
      # Check initial node count
      expect(Ezine::Node::Base.count).to eq(0)
      ...
      expect(Ezine::Node::Base.count).to eq(csv.size)

      Ezine::Node::Base.each_with_index do |node, index|
