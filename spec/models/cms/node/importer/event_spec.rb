...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:cate1) { create :category_node_page, name: "文化・芸術", filename: "bunka" }
  let!(:cate2) { create :category_node_page, name: "講演・講座 ", filename: "kohen" }
  let!(:cate3) { create :category_node_page, name: "スポーツ", filename: "sports" }

  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/event.csv" }
  ...
  context "event nodes" do
    it "#import" do
      # Check initial node count
      expect(Event::Node::Base.count).to eq(0)
      ...
      expect(Event::Node::Base.count).to eq(csv.size)
