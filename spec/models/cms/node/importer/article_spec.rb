...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:cate1) { create :category_node_page, name: "避難所", filename: "hinanjo" }
  let!(:cate2) { create :category_node_page, name: "土砂災害", filename: "hinanjo/dosya" }
  let!(:cate3) { create :category_node_page, name: "地震", filename: "hinanjo/jishin" }
  let!(:cate4) { create :category_node_page, name: "津波", filename: "hinanjo/thunami" }
  ...
  context "article nodes" do
    it "#import" do
      ...
      expect(Article::Node::Base.count).to eq 0
      ...
      expect(Article::Node::Base.count).to eq(csv.size)

      # id order was reversed because overwritten hinanjo node
      nodes = Article::Node::Base.all.to_a
      nodes = nodes.reverse
