...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/chat.csv" }
  ...
  context "chat nodes" do
    it "#import" do
      # Check initial node count
      expect(Chat::Node::Base.count).to eq 0
      ...
      expect(Chat::Node::Base.count).to eq(csv.size)

      Chat::Node::Base.each_with_index do |node, index|
