...
describe Cms::NodeImporter, dbscope: :example do
  ...
  let!(:csv_path) { "#{Rails.root}/spec/fixtures/cms/node/import/inquiry.csv" }
  ...
  context "inquiry nodes" do
    it "#import" do
      # Check initial node count
      expect(Inquiry::Node::Base.count).to eq 0
      ...
      expect(Inquiry::Node::Base.count).to eq(csv.size)

      Inquiry::Node::Base.each_with_index do |node, index|
