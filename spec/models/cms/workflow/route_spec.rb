...
describe Cms::Workflow::Route do
  describe "#approvers" do
    ...
    context "when array of hash is given" do
      subject { Cms::Workflow::Route.new({ approvers: [ nil, { level: 1, user_id: 2 }, { level: 2, user_id: 1 }, { level: 1, user_id: 2 } ] }) }
      it { expect(subject.approvers.length).to eq 2 }
      it { expect(subject.approvers[0][:level]).to eq 1 }
      it { expect(subject.approvers[0][:user_id]).to eq 2 }
      it { expect(subject.approvers[1][:level]).to eq 2 }
  ...
  describe "#required_counts" do
    context "when csv is given" do
      subject { Cms::Workflow::Route.new({ required_counts: [ "false", "1", "2" ] }) }
  ...
  describe "#validate" do
    context "when name is missing" do
      subject { Cms::Workflow::Route.new({ group_ids: [ 1 ],
                                           approvers: [ "", "1,2", "2,1", "1,2" ],
                                           required_counts: [ "false", "1", "2" ] }) }
      it { expect(subject).to have(1).errors_on(:name) }
    end

    context "when group_ids is missing" do
      subject { Cms::Workflow::Route.new({ name: "workflow-#{rand(0x100000000).to_s(36)}",
                                           approvers: [ "", "1,2", "2,1", "1,2" ],
                                           required_counts: [ "false", "1", "2" ] }) }
  ...
  describe "#search" do
    ...
    context "when name is given" do
      subject { Cms::Workflow::Route.search({ name: rand(0x100000000).to_s(36) }) }
      it { expect(subject).not_to be_nil }
      it { expect(subject.count).to eq 0 }
    end

    context "when keyword is given" do
      subject { Cms::Workflow::Route.search({ keyword: rand(0x100000000).to_s(36) }) }
      it { expect(subject).not_to be_nil }
      it { expect(subject.count).to eq 0 }
    end
  end
