  ...
  describe "#condition_hash" do
    context "on a node 'cms/node'" do
      context "with empty conditions" do
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [] }
      ...
      context "with existing node" do
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [ article_node.filename ] }
        subject { node.condition_hash["$or"] }
      ...
      context "with non-existing node" do
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [ "node-#{unique_id}" ] }
      ...
      context "with non-existing node as wildcard" do
        ...
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [ "#{filename}/*" ] }
        subject { node.condition_hash["$or"] }
      ...
      context "when \#{request_dir} is given with blank cur_main_path" do
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [ "\#{request_dir}" ] }
        subject do
        ...
        it do
          # #{request_dir} is not supported at facility/node/search#condition_hash
          expect(subject).to be_a(Array)
          expect(subject.length).to eq 1
          expect(subject[0]).to eq(id: -1)
        end
      ...
      context "when \#{request_dir} with sub directory is given with actual cur_main_path" do
        ...
        let!(:node) { create :facility_node_search, cur_site: site, layout: layout, conditions: [ condition ] }
        subject do
