...
describe Cms::Column::Value::Select, type: :model, dbscope: :example do
  describe "what cms/column/value/select exports to liquid" do
    let!(:node) { create :article_node_page }
    let!(:form) { create(:cms_form, cur_site: cms_site, state: 'public', sub_type: 'static') }
    let!(:column1) { create(:cms_column_select, cur_form: form, order: 1) }
    let!(:page) do
      create(
