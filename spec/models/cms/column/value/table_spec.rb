...
describe Cms::Column::Value::Table, type: :model, dbscope: :example do
  describe "what cms/column/value/table exports to liquid" do
    let!(:node) { create :article_node_page }
    let!(:form) { create(:cms_form, cur_site: cms_site, state: 'public', sub_type: 'static') }
    let!(:column1) { create(:cms_column_table, cur_form: form, order: 1) }
    let!(:page) do
      create(
    ...
    it do
      ...
      expect(subject.html).to eq value.value.gsub("\n", "<br />")
