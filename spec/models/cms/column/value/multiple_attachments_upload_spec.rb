...
describe Cms::Column::Value::MultipleAttachmentsUpload, type: :model, dbscope: :example do
  let!(:site) { cms_site }
  ...
  describe "#header field" do
    ...
    it "persists the header text alongside file_ids" do
      ...
      expect(value.header).to eq "詳細については下記をご覧ください。"
    ...
    it "includes the header in history_summary" do
      page.reload
      value = page.column_values.first
      expect(value.history_summary).to include("詳細については下記をご覧ください。")
    end

    it "matches header in search_values" do
      page.reload
      value = page.column_values.first
      expect(value.search_values(["詳細については下記をご覧ください。"])).to be_truthy
    end
  end
