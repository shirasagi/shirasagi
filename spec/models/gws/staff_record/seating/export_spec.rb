  ...
  context "with Shift_JIS" do
    ...
    it do
      ...
      if SS.config.ss.disable_permission_level
        ...
        expect(csv.headers.length).to eq 11
      end
      basic_headers = %i[id name url remark order].map { |f| described_class.t(f) }
      ...
      csv.first.tap do |row|
        ...
        expect(row[described_class.t(:id)]).to eq staff_record_seating1.id.to_s
        expect(row[described_class.t(:name)]).to eq staff_record_seating1.name
        expect(row[described_class.t(:url)]).to eq staff_record_seating1.url
        expect(row[described_class.t(:remark)]).to eq staff_record_seating1.remark
        expect(row[described_class.t(:order)]).to eq staff_record_seating1.order.to_s
        expect(row[described_class.t(:readable_setting_range)]).to eq staff_record_seating1.label(:readable_setting_range)
        expect(row[described_class.t(:readable_group_ids)]).to eq staff_record_seating1.readable_groups.pluck(:name).join("\n")
        expect(row[described_class.t(:readable_member_ids)]).to eq staff_record_seating1.readable_members.pluck(:uid).join("\n")
        expect(row[described_class.t(:group_ids)]).to eq staff_record_seating1.groups.pluck(:name).join("\n")
        expect(row[described_class.t(:user_ids)]).to eq staff_record_seating1.users.pluck(:uid).join("\n")
        unless SS.config.ss.disable_permission_level
          expect(row[described_class.t(:permission_level)]).to eq staff_record_seating1.permission_level.to_s
        end
      end
    end
  end
