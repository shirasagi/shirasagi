...
describe Gws::Member, type: :model, dbscope: :example do
  ...
  let(:new_item) { build :gws_schedule_plan, cur_site: site }
  let(:init) do
  ...
  context 'member_ids are set' do
    it do
      ...
      # check methods
      expect(item.member_include?(user1)).to be_truthy
      expect(item.member_include?(user2)).to be_falsey
      expect(item.member_include?(user3)).to be_truthy
      expect(item.member_include?(user4)).to be_truthy
      ...
      expect(item.overall_members_previously_was.count).to eq 1
      expect(item.overall_members_previously_was.pluck(:id)).to include(gws_user.id)
      ...
      expect(item.sorted_overall_members_previously_was.count).to eq 1
      expect(item.sorted_overall_members_previously_was.pluck(:id)).to include(gws_user.id)
    end
  end
  ...
  context 'member_group_ids are set' do
    it do
      ...
      # check methods
      expect(item.member_include?(user1)).to be_falsey
      expect(item.member_include?(user2)).to be_truthy
      expect(item.member_include?(user3)).to be_truthy
      expect(item.member_include?(user4)).to be_truthy
  ...
  context 'member_custom_groups contain users are set' do
    it do
      ...
      expect(item.member_include?(user1)).to be_truthy
      expect(item.member_include?(user2)).to be_truthy
      expect(item.member_include?(user3)).to be_falsey
      expect(item.member_include?(user4)).to be_truthy
      expect(item.sorted_members).to be_blank
