...
describe Gws::Member, type: :model, dbscope: :example do
  ...
  let(:custom_group) { create :gws_custom_group, member_ids: [user1.id, user2.id] }
  ...
  context 'member_ids are set' do
    it do
      # empty
      item.update_attributes(init.merge(member_ids: [user1.id, user3.id]))
      ...
      expect(item.sorted_members.pluck(:id)).to include(user1.id, user3.id)
      expect(item.sorted_members.pluck(:id)).not_to include(user2.id)
      expect(item.overall_members.pluck(:id)).to include(user1.id, user3.id)
  ...
  context 'member_group_ids are set' do
    it do
      # empty
      item.update_attributes(init.merge(member_group_ids: [group2.id, group3.id]))
      expect(item.valid?).to be_truthy
      ...
      expect(item.overall_members.pluck(:id)).to include(user2.id, user3.id)
  ...
  context 'member_custom_group_ids are set' do
    it do
      # empty
      item.update_attributes(init.merge(member_custom_group_ids: [custom_group.id]))
      expect(item.valid?).to be_truthy
      expect(item.member_ids).to be_blank
      expect(item.member_group_ids).to be_blank
      ...
      expect(item.sorted_members).to be_blank
      expect(item.overall_members.pluck(:id)).to include(user1.id, user2.id)
