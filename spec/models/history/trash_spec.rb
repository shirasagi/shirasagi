...
describe History::Trash do
  ...
  context 'when destroy pages with file' do
    ...
    it "restore" do
      item.destroy
      expect(File.exists?("#{Rails.root}/private/trash/#{file.path.sub(/.*\/(ss_files\/)/, '\\1')}")).to be_truthy

      described_class.all.destroy_all
      expect(File.exists?("#{Rails.root}/private/trash/#{file.path.sub(/.*\/(ss_files\/)/, '\\1')}")).to be_falsey
    end
  end
end
