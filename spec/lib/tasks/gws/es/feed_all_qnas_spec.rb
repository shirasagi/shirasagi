...
describe Tasks::Gws::Es, dbscope: :example, es: true do
  ...
  describe ".feed_all_qnas" do
    ...
    let!(:topic1) do
      create(:gws_qna_topic, cur_site: site, cur_user: user, category_ids: [category.id], file_ids: [file1.id])
    end
    let!(:post1) { create(:gws_qna_post, cur_site: site, cur_user: user, topic: topic1, parent: topic1) }

    let!(:file2) do
      tmp_ss_file(user: user, contents: "#{Rails.root}/spec/fixtures/ss/logo.png", binary: true, content_type: 'image/png')
    end
    let!(:topic2) do
      create(:gws_qna_topic, cur_site: site, cur_user: user, category_ids: [category.id], file_ids: [file2.id])
    end
    let!(:post2) { create(:gws_qna_post, cur_site: site, cur_user: user, topic: topic2, parent: topic2) }
    ...
    let!(:deleted_post) do
      create(:gws_qna_post, cur_site: site, cur_user: user, topic: topic1, parent: topic1, deleted: now)
    end
    ...
    it do
      expect { described_class.feed_all_qnas }.to output(include(post1.name, post2.name)).to_stdout
      ...
      site.elasticsearch_client.search(index: "g#{site.id}", size: 100, q: "*:*").tap do |es_docs|
        ...
        expect(ids).to include(
          "gws_qna_posts-post-#{topic1.id}", "file-#{file1.id}", "gws_qna_posts-post-#{post1.id}",
          "gws_qna_posts-post-#{topic2.id}", "file-#{file2.id}", "gws_qna_posts-post-#{post2.id}")
      end
    end
  end
end
