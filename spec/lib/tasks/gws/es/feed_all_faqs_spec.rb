...
describe Tasks::Gws::Es, dbscope: :example, es: true do
  ...
  describe ".feed_all_faqs" do
    ...
    let!(:deleted_post) do
      create(:gws_faq_post, cur_site: site, cur_user: user, topic: topic, parent: topic, deleted: now)
    end
    ...
    it do
      ...
      site.elasticsearch_client.search(index: "g#{site.id}", size: 100, q: "*:*").tap do |es_docs|
        ...
        es_docs["hits"]["hits"][2].tap do |es_doc|
          expect(es_doc["_id"]).to eq "gws_faq_posts-post-#{post.id}"
          source = es_doc["_source"]
          expect(source['url']).to eq "/.g#{site.id}/faq/-/-/topics/#{topic.id}#post-#{post.id}"
        end
      end
    end
  end
