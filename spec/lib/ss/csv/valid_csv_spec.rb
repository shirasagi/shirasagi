...
describe SS::Csv do
  ...
  context "with UTF-8 (with BOM) csv" do
    let(:csv_file) { "#{Rails.root}/spec/fixtures/cms/node/import/article.csv" }
    ...
    context "with valid required_headers" do
      let(:required_headers) do
        [ Article::Page.t(:filename), Article::Page.t(:name) ]
      end
  ...
  context "with UTF-16LE (with BOM) csv" do
    let(:csv_file) { "#{Rails.root}/spec/fixtures/ss/csv/article_utf16le.csv" }
