...
describe "multilingual_node_pages", dbscope: :example do
  ...
  let!(:node) { create_once :event_node_page, filename: "docs", name: "event" }
  let!(:native_item) { create(:event_page, cur_node: node) }
  let!(:foreign_item) { create(:event_page, filename: "#{lang}/#{native_item.filename}") }
