...
describe "cms_apis_files", type: :feature, dbscope: :example do
  ...
  context "with auth" do
    ...
    it "#new" do
      ...
      SS::File.all.first.tap do |file|
        ...
        expect(file).to be_a(Cms::File)
        expect(file.site_id).to eq site.id
        expect(file.user_id).to eq cms_user.id
        expect(file.group_ids).to eq cms_user.group_ids
  ...
  context "when filename is use multibyte character" do
    let(:path) { "#{Rails.root}/spec/fixtures/ss/ロゴ.png" }
    before { login_cms_user }
