...
describe "cms_parts", type: :feature, dbscope: :example, js: true do
  ...
  context "shows accessibility errors and link errors" do
    ...
    context "#edit" do
      it do
        ...
        Cms::Part.find(item.id).tap do |after_item|
  ...
  context "with accessibility error and permissions" do
    ...
    it "shows accessibility error and allows ignore if permitted" do
      ...
      Cms::Part.where(name: "アクセシビリティテスト").first.tap do |part|
        ...
        # アクセシビリティチェックを実行すると、常に結果が保存/更新される
        expect(part.syntax_check_result_checked.in_time_zone).to be_within(30.seconds).of(Time.zone.now)
        expect(part.syntax_check_result_violation_count).to eq 1
      end
    end
    ...
    it "auto correct removes error" do
      ...
      Cms::Part.where(name: "自動修正テスト").first.tap do |part|
        ...
        # アクセシビリティチェックを実行すると、常に結果が保存/更新される
        expect(part.syntax_check_result_checked.in_time_zone).to be_within(30.seconds).of(Time.zone.now)
        expect(part.syntax_check_result_violation_count).to eq 0
      end
    end
  end
  ...
  context "continuous correction" do
    ...
    it do
      if new_or_edit == :new
        login_user admin, to: new_cms_part_path(site: site)
      else
        item = create(:cms_part_free, cur_site: site)
        login_user admin, to: edit_cms_part_path(site: site, id: item)
      end
      within "form#item-form" do
        fill_in_code_mirror "item[html]", with: html_with_error
