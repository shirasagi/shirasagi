  ...
  context "without current node" do
    it do
      ...
      within "#cms-nodes-tree-frame" do
        ...
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node1.id}']", text: node1.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2.id}']", text: node2.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_1.id}']", text: node2_1.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_2.id}']", text: node2_2.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_1_1.id}']", text: node2_1_1.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_1_2.id}']", text: node2_1_2.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_2_1.id}']", text: node2_2_1.name)
        expect(page).to have_css(".ss-tree-item[data-node-id='#{node2_2_2.id}']", text: node2_2_2.name)
      end
    end
  end
  ...
  context "with node2" do
    it do
      ...
      within "#cms-nodes-tree-frame" do
        ...
        expect(page).to have_css(".ss-tree-item.is-current[data-node-id='#{node2.id}']", text: node2.name)

        # 自フォルダーは展開されている
        first(".ss-tree-item[data-node-id='#{node2.id}']").tap do |el|
          wrap_el = el.evaluate_script("this.querySelector('.ss-tree-subtree-wrap')")
          expect(wrap_el.evaluate_script('this.open')).to be_truthy
        end
        # 子孫フォルダーは展開されていない
        first(".ss-tree-item[data-node-id='#{node2_1.id}']").tap do |el|
          wrap_el = el.evaluate_script("this.querySelector('.ss-tree-subtree-wrap')")
          expect(wrap_el.evaluate_script('this.open')).to be_falsey
        end
        first(".ss-tree-item[data-node-id='#{node2_2.id}']").tap do |el|
          wrap_el = el.evaluate_script("this.querySelector('.ss-tree-subtree-wrap')")
          expect(wrap_el.evaluate_script('this.open')).to be_falsey
        end
      end
  ...
  context "with node2_1" do
    it do
      ...
      within "#cms-nodes-tree-frame" do
        ...
        expect(page).to have_css(".ss-tree-item.is-current[data-node-id='#{node2_1.id}']", text: node2_1.name)
  ...
  context "with node2_2_2" do
    it do
      ...
      within "#cms-nodes-tree-frame" do
        ...
        expect(page).to have_css(".ss-tree-item.is-current[data-node-id='#{node2_2_2.id}']", text: node2_2_2.name)
