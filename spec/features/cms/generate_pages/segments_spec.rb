...
describe "cms_generate_pages", type: :feature, dbscope: :example, js: true do
  ...
  let(:index_path) { cms_generate_pages_path site.id }
  let(:web01_path) { cms_segment_generate_pages_path site.id, segment: "web01" }
  let(:web02_path) { cms_segment_generate_pages_path site.id, segment: "web02" }
  let(:web03_path) { cms_segment_generate_pages_path site.id, segment: "web03" }

  def find_task(segment)
    Cms::Task.site(site).where(name: "cms:generate_pages", node_id: nil, segment: segment).first
  end
