...
describe 'cms_agents_nodes_archive', type: :feature, dbscope: :example, js: true do
  ...
  let!(:archive_node) { create :cms_node_archive, cur_node: root_node, layout: layout, archive_view: "calendar" }
  ...
  context 'visit monthly' do
    ...
    it do
      ...
      expect(page).to have_css(".event-date", text: title2)
      within "#event-list" do
        expect(page).to have_selector(".page", count: 1)
        within first(".page") do
          expect(page).to have_link item2.name
        end
      end

      visit "#{archive_node.url}#{month1.strftime('%Y%m')}/"
      expect(page).to have_css(".event-date", text: title1)
      within "#event-list" do
        expect(page).to have_selector(".page", count: 1)
        within first(".page") do
      ...
      expect(page).to have_css(".event-date", text: title3)
      within "#event-list" do
        expect(page).to have_selector(".page", count: 1)
        within first(".page") do
