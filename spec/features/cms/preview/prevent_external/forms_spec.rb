...
describe "cms_preview", type: :feature, dbscope: :example, js: true do
  ...
  let!(:node1) { create :cms_node_node, cur_site: site, filename: "search", layout: layout }
  let!(:node2) { create :cms_node_node, cur_site: subsite1, filename: "search", layout: layout }
  let!(:node3) { create :cms_node_node, cur_site: subsite2, filename: "search", layout: layout }

  let!(:html) do
    h = Fs.read("#{Rails.root}/spec/fixtures/cms/preview/forms.html")
  ...
  context "preview in root site" do
    ...
    it "submit get forms" do
      visit cms_preview_path(site: site, path: top_page1.preview_path)
      first(".get-top input[type=\"submit\"]").click
      expect(page).to have_css(".cms-nodes.nodes")
      expect(current_path).to eq cms_preview_path(site: site, path: node1.preview_path)

      visit cms_preview_path(site: site, path: top_page1.preview_path)
      page.accept_alert(I18n.t("cms.notices.prevent_external_preview")) do
        first(".get-sub1 input[type=\"submit\"]").click
      end
      expect(current_path).to eq cms_preview_path(site: site, path: top_page1.preview_path)

      page.accept_alert(I18n.t("cms.notices.prevent_external_preview")) do
        first(".get-sub2 input[type=\"submit\"]").click
      end
      expect(current_path).to eq cms_preview_path(site: site, path: top_page1.preview_path)
    end

    it "submit post forms" do
      visit cms_preview_path(site: site, path: top_page1.preview_path)
      page.accept_alert(I18n.t("cms.notices.prevent_external_submit")) do
        first(".post-top input[type=\"submit\"]").click
      end
      expect(current_path).to eq cms_preview_path(site: site, path: top_page1.preview_path)

      page.accept_alert(I18n.t("cms.notices.prevent_external_submit")) do
        first(".post-sub1 input[type=\"submit\"]").click
      end
      expect(current_path).to eq cms_preview_path(site: site, path: top_page1.preview_path)

      page.accept_alert(I18n.t("cms.notices.prevent_external_submit")) do
        first(".post-sub2 input[type=\"submit\"]").click
      end
      expect(current_path).to eq cms_preview_path(site: site, path: top_page1.preview_path)
    end
  end
  ...
  context "preview in subsite1" do
    ...
    it "submit get forms" do
      ...
      first(".get-sub1 input[type=\"submit\"]").click
      expect(page).to have_css(".cms-nodes.nodes")
      expect(current_path).to eq cms_preview_path(site: subsite1, path: node2.preview_path)
