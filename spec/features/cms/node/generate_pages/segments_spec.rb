...
describe "node_generate_pages", type: :feature, dbscope: :example, js: true do
  let(:site) { cms_site }
  ...
  def find_task
    Cms::Task.site(site).where(name: "cms:generate_pages", node_id: node.id, segment: nil).first
  end
