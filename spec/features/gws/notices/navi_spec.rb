...
describe "gws_notice_folders", type: :feature, dbscope: :example, js: true do
  ...
  it do
    ...
    within '.tree-navi' do
      expect(page).to have_css(".tree-item[data-id='#{public_folder.id}']", text: public_folder.name)
      expect(page).to have_no_css(".tree-item[data-id='#{limited_folder.id}']")
    end
    within '.list-items' do
      expect(page).to have_css(".list-item[data-id='#{post_under_public.id}']", text: post_under_public.name)
      expect(page).to have_no_css(".list-item[data-id='#{post_under_limited.id}']")
    end
    ...
    within '.tree-navi' do
      expect(page).to have_css(".tree-item[data-id='#{public_folder.id}']", text: public_folder.name)
      expect(page).to have_css(".tree-item[data-id='#{limited_folder.id}']", text: limited_folder.name)
    end
    within '.list-items' do
      expect(page).to have_css(".list-item[data-id='#{post_under_public.id}']", text: post_under_public.name)
