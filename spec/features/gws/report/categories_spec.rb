...
describe "gws_report_categories", type: :feature, dbscope: :example, js: true do
  ...
  context "delete parent" do
    let!(:category0) { create :gws_report_category, cur_site: site }
    let!(:category1) { create :gws_report_category, cur_site: site, name: "#{category0.name}/#{unique_id}" }

    it do
      login_gws_user to: gws_report_categories_path(site: site)
      ...
      wait_for_error I18n.t("mongoid.errors.models.gws/report/category.found_children")
    end
  end
end
