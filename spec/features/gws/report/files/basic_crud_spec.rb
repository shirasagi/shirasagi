...
describe "gws_report_files", type: :feature, dbscope: :example, js: true do
  ...
  context "basic crud" do
    let(:name) { unique_id }
    let(:name2) { unique_id }
    let(:column_value1_1) { unique_id }
    let(:column_value1_2) { "#{unique_id}@example.jp" }
    let(:column_value1_3) { Array.new(8) { rand(10).to_s }.join }
    let(:column_value2_1) { Time.zone.now.change(hour: 0, min: 0, sec: 0) }
    let(:column_value2_2) { Time.zone.now.change(min: 0, sec: 0) }
    ...
    it do
      ...
      within "form#item-form" do
        ...
        fill_in "custom[#{column1_1.id}]", with: column_value1_1
        fill_in "custom[#{column1_2.id}]", with: column_value1_2
        fill_in "custom[#{column1_3.id}]", with: column_value1_3
        fill_in "custom[#{column2_1.id}]", with: column_value2_1.strftime("%Y/%m/%d")
        fill_in "custom[#{column2_2.id}]", with: column_value2_2.strftime("%Y/%m/%d %H:%M")
      ...
      file.column_values.where(column_id: column1_1.id).first.tap do |cv|
        expect(cv.value).to eq column_value1_1
      end
      file.column_values.where(column_id: column1_2.id).first.tap do |cv|
        expect(cv.value).to eq column_value1_2
      end
      file.column_values.where(column_id: column1_3.id).first.tap do |cv|
        expect(cv.value).to eq column_value1_3
      end
      file.column_values.where(column_id: column2_1.id).first.tap do |cv|
        expect(cv.date).to eq column_value2_1
      end
      file.column_values.where(column_id: column2_2.id).first.tap do |cv|
        expect(cv.date).to eq column_value2_2
      end
