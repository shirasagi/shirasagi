...
describe "gws_elasticsearch_search_circular", type: :feature, dbscope: :example, js: true, es: true do
  let(:site) { gws_site }
  let(:now) { Time.zone.now.beginning_of_minute }
  let(:user) { gws_user }
  let(:index_path) { gws_elasticsearch_search_search_path(site: site.id, type: 'circular') }
