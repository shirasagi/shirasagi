...
describe "gws_discussion_todos", type: :feature, dbscope: :example, js: true do
  ...
  let(:clone_name) { "[#{I18n.t("workflow.cloned_name_prefix")}] [#{forum.name}]" }
  ...
  it do
    ...
    within "#addon-basic" do
      expect(page).to have_text(clone_name)
    end
    ...
    wait_for_js_ready
    expect(page).to have_css('.fc-view a.fc-event-todo', text: clone_name)
  end
end
