...
describe "gws_qna_topics", type: :feature, dbscope: :example do
  context "comments" do
    ...
    context "with thread topic" do
      let(:topic) { create :gws_qna_topic, mode: 'thread' }
    ...
    context "with tree topic" do
      let(:topic) { create :gws_qna_topic, mode: 'tree' }
      let(:name2) { unique_id }
      let(:text2) { unique_id }
      ...
      it "#show" do
        ...
        comment = Gws::Qna::Post.where(topic_id: topic.id).order_by(created: -1).first
