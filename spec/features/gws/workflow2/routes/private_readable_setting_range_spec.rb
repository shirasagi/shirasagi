...
describe "gws_workflow2_routes", type: :feature, dbscope: :example, js: true do
  ...
  context "edit the route which readable_setting_range is 'private'" do
    let(:name1_ja) { unique_id }
    let(:remark1) { Array.new(2) { unique_id } }
    let(:name2_ja) { unique_id }
    ...
    it do
      ...
      within "form#item-form" do
        fill_in "item[i18n_name_translations][#{I18n.default_locale}]", with: name1_ja
      ...
      Gws::Workflow2::Route.all.first.tap do |route|
        ...
        expect(route.i18n_name_translations[I18n.default_locale]).to eq name1_ja
      ...
      within "form#item-form" do
        fill_in "item[i18n_name_translations][#{I18n.default_locale}]", with: name2_ja
      ...
      Gws::Workflow2::Route.all.first.tap do |route|
        expect(route.site_id).to eq site.id
        expect(route.i18n_name_translations[I18n.default_locale]).to eq name2_ja
