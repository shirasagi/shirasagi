...
describe "gws_workflow2_routes", type: :feature, dbscope: :example, js: true do
  let!(:site) { gws_site }
  let(:name_ja) { unique_id }
  let(:name_en) { unique_id }
  ...
  context "with superior" do
    ...
    it do
      ...
      expect(route.i18n_name_translations[:ja]).to eq name_ja
      expect(route.i18n_name_translations[:en]).to eq name_en
