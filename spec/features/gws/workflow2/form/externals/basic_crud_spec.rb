  ...
  context "basic crud" do
    ...
    it do
      ...
      within "form#item-form" do
        fill_in "item[i18n_name_translations][#{I18n.default_locale}]", with: name
        fill_in "item[order]", with: order
        fill_in "item[i18n_url_translations][#{I18n.default_locale}]", with: url
        fill_in "item[i18n_description_translations][#{I18n.default_locale}]", with: description.join("\n")
      ...
      Gws::Workflow2::Form::Base.all.first.tap do |item|
        ...
        expect(item.i18n_name).to eq name
        expect(item.i18n_description).to eq description.join("\r\n")
        expect(item.i18n_name_translations[I18n.default_locale]).to eq name
        expect(item.i18n_description_translations[I18n.default_locale]).to eq description.join("\r\n")
        expect(item.i18n_name_translations[:en]).to be_blank
        expect(item.i18n_description_translations[:en]).to be_blank
        expect(item.order).to eq order
        expect(item.i18n_url).to eq url
        expect(item.i18n_url_translations[I18n.default_locale]).to eq url
        expect(item.i18n_url_translations[:en]).to be_blank
      ...
      within "form#item-form" do
        fill_in "item[i18n_name_translations][#{I18n.default_locale}]", with: name2
        click_on I18n.t("ss.buttons.save")
      end
      wait_for_notice I18n.t("ss.notice.saved")
      ...
      Gws::Workflow2::Form::Base.all.first.tap do |item|
        ...
        expect(item.i18n_name).to eq name2
        expect(item.i18n_name_translations[I18n.default_locale]).to eq name2
        expect(item.i18n_name_translations[:en]).to be_blank
      end
