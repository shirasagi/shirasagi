...
describe "gws_workflow2_form_applications", type: :feature, dbscope: :example, js: true do
  ...
  let!(:form) do
    form = create(
      :gws_workflow2_form_application, cur_site: site, category_ids: [ category1.id ], purpose_ids: [ purpose1.id ],
      destination_user_ids: [ dest_user.id ], state: "public")
  ...
  context "copy" do
    it do
      ...
      expect(Gws::Workflow2::Form::Base.all.count).to eq 2
      Gws::Workflow2::Form::Base.all.ne(id: form.id.to_s).first.tap do |copied_form|
        expect(copied_form).to be_a(Gws::Workflow2::Form::Application)
        expect(copied_form.id.to_s).not_to eq form.id.to_s
        expect(copied_form.site_id).to eq form.site_id

        I18n.available_locales.each do |lang|
          prefix = I18n.with_locale(lang) { I18n.t("gws/notice.prefix.copy") }
          expect(copied_form.i18n_name_translations[lang]).to eq "#{prefix} #{form.i18n_name_translations[lang]}"
          expect(copied_form.i18n_description_translations[lang]).to eq form.i18n_description_translations[lang]
        end
        expect(copied_form.order).to eq form.order
        expect(copied_form.state).to eq "closed"
        expect(copied_form.approval_state).to eq form.approval_state
        expect(copied_form.default_route_id).to eq form.default_route_id
        ...
        expect(copied_form.purpose_ids).to eq form.purpose_ids
        expect(copied_form.destination_group_ids).to eq form.destination_group_ids
        expect(copied_form.destination_user_ids).to eq form.destination_user_ids
