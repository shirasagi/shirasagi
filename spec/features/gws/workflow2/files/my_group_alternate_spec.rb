...
describe Gws::Workflow2::FilesController, type: :feature, dbscope: :example, js: true do
  ...
  context "my group_alternate with single superior" do
    let(:site) { gws_site }
    let(:admin) { gws_user }
    ...
    it do
      ...
      within ".workflow_approvers" do
        expect(page).to have_css(".gws-workflow-file-approver-item[data-level='1']", text: user1.long_name)
  ...
  # 上長が複数存在する場合、'my_group_alternate'はうまく動作しないので一時的に無効化
  xcontext "my group_alternate with multiple superiors" do
    let(:site) { gws_site }
    let(:admin) { gws_user }
    let!(:minimum_role) { create(:gws_role, cur_site: site, permissions: %w(use_gws_workflow2)) }
    let!(:user1) { create(:gws_user, :gws_workflow_notice, group_ids: admin.group_ids, gws_role_ids: [minimum_role.id]) }
    ...
    before do
      ...
      admin.groups.first.set(superior_user_ids: [user1.id, user2.id])
    end
    ...
    it do
      ...
      within ".workflow_approvers" do
        expect(page).to have_css(".gws-workflow-file-approver-item[data-level='1']", text: user1.long_name)
        expect(page).to have_css(".gws-workflow-file-approver-item[data-level='1']", text: user2.long_name)
        wait_for_cbox_opened { click_on I18n.t("gws/workflow2.search_alternates.index") } ##
      end
      within_cbox do
      ...
      expect(item.workflow_approvers).to include(
        ...
        { level: 1, user_type: "Gws::User", user_id: user3.id, state: 'request', comment: '' }
      )
      expect(item.workflow_required_counts).to eq [ 1 ]
      expect(item.workflow_circulations.count).to eq 0
      ...
      SS::Notification.order_by(id: -1).first.tap do |memo|
        expect(memo.user_id).to eq admin.id
        expect(memo.member_ids).to eq [user1.id, user2.id, user3.id]
        subject = I18n.t("gws_notification.gws/workflow/file.request", name: item.name)
        expect(memo.subject).to eq subject
      end
      ...
      expect(item.workflow_approvers).to include(
        ...
        { level: 1, user_type: "superior", user_id: user2.id, state: 'other_approved', comment: '',
          created: be_within(30.seconds).of(Time.zone.now) },
        { level: 1, user_type: "Gws::User", user_id: user3.id, state: 'approve', comment: approve_comment1,
          file_ids: nil, created: be_within(30.seconds).of(Time.zone.now) }
      )
      expect(item.workflow_required_counts).to eq [ 1 ]
      expect(item.workflow_circulations.count).to eq 0
