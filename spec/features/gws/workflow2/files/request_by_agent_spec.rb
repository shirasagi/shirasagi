    ...
    context "with route alternatable superior" do
      ...
      let!(:route) do
        create(
          :gws_workflow2_route, group_ids: admin.group_ids,
          approvers: [
            { "level" => 1, "user_type" => "superior", "user_id" => "superior", "alternatable" => 1 }
          ],
          required_counts: [ false ]
        )
      end
      ...
      it do
        ...
        within_cbox do
          within "form.search" do
            click_on user1_group.name
            within first(".dropdown-container") do
              click_on user2_group.trailing_name
            end
          end
        end
        within_cbox do
          wait_for_cbox_closed { click_on alternate_user.long_name }
        end
        within ".mod-workflow-request" do
          within ".gws-workflow-file-approver-item[data-level='1']" do
            expect(page).to have_css("[data-id='#{user2_superior.id}']", text: user2_superior.long_name)
            expect(page).to have_css("[data-id='#{user2_superior.id}-alternative']", text: alternate_user.long_name)
          end
        ...
        within ".mod-workflow-view" do
          ...
          expect(page).to have_css(".workflow_state", text: I18n.t("workflow.state.request"))
          expect(page).to have_css(".workflow_comment", text: workflow_comment)
          expect(page).to have_css(".workflow_approvers [data-level='1'][data-user-id='#{user2_superior.id}']")
          expect(page).to have_css(".workflow_approvers [data-level='1'][data-user-id='#{alternate_user.id}']")
        end
        ...
        Gws::Workflow2::File.all.first.tap do |item|
          ...
          expect(item.workflow_approvers).to include(
            { level: 1, user_type: "superior", user_id: user2_superior.id, state: 'request', comment: be_blank },
            { level: 1, user_type: "superior", user_id: alternate_user.id, alternate_to: "1,superior,#{user2_superior.id}",
              state: 'request', comment: be_blank, })
          expect(item.workflow_circulations).to be_blank
        end
        ...
        SS::Notification.order_by(id: -1).to_a.tap do |notifications|
          notifications[0].tap do |notification|
            ...
            expect(notification.html).to be_blank
            expect(notification.user_id).to eq user2.id
            expect(notification.member_ids).to eq [user2_superior.id, alternate_user.id]
          end
        end

        #
        # alternate_user: 申請を承認する（代理承認者 ）
        #
        login_user alternate_user, to: gws_workflow2_files_path(site: site, state: "all")
        click_on file.name
        wait_for_turbo_frame "#workflow-approver-frame"
        within ".mod-workflow-view" do
          expect(page).to have_css(".workflow_state", text: I18n.t("workflow.state.request"))
        ...
        within ".mod-workflow-approve" do
          ...
          I18n.t("gws/workflow2.notice.use_approver_alternator").each do |check_box_text|
            check check_box_text
          end
        ...
        Gws::Workflow2::File.all.first.tap do |item|
          ...
          expect(item.workflow_approvers.count).to eq 2
          expect(item.workflow_approvers).to include(
            { level: 1, user_type: "superior", user_id: user2_superior.id, state: 'other_approved', comment: be_blank,
              created: be_within(30.seconds).of(Time.zone.now) },
            { level: 1, user_type: "superior", user_id: alternate_user.id, alternate_to: "1,superior,#{user2_superior.id}",
              state: 'approve', comment: approve_comment1, file_ids: be_blank, created: be_within(30.seconds).of(Time.zone.now) })
          expect(item.workflow_circulations).to be_blank
        end
        ...
        SS::Notification.order_by(id: -1).to_a.tap do |notifications|
          notifications[0].tap do |notification|
            ...
            expect(notification.text).to be_blank
            expect(notification.html).to be_blank
            expect(notification.user_id).to eq alternate_user.id
            expect(notification.member_ids).to include(user1.id, user2.id)
          end
        end
      end
