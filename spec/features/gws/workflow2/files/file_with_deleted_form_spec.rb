...
describe Gws::Workflow2::FilesController, type: :feature, dbscope: :example, js: true do
  ...
  # 全部入りのフォーム
  let!(:form) { create(:gws_workflow2_form_application, state: "public", destination_user_ids: [ dest_user.id ]) }
  ...
  let!(:item) do
    ...
    create(
      :gws_workflow2_file, cur_user: user, form: form, destination_user_ids: form.destination_user_ids,
      column_values: [
  ...
  context "sort delete all with approved" do
    it do
      ...
      wait_for_notice I18n.t("ss.notice.unable_to_delete", items: item.name)
