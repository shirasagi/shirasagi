  ...
  context "approve file" do
    ...
    let!(:user1) do
      Gws::User.create name: "一般ユーザー1", uid: "user1", email: "user1@example.jp", in_password: "pass",
        group_ids: [ group1.id ], gws_role_ids: admin.gws_role_ids,
    ...
    context "superior user is automatically set if group is set superior user" do
      it do
        group1.update!(superior_user_ids: [ admin.id ])
        login_user user1
        visit show_path
        wait_for_js_ready
        ...
        expect(item.workflow_approvers).to \
          include({level: 1, user_type: "superior", user_id: admin.id, state: 'request', comment: ''})
        ...
        if admin.notice_workflow_user_setting == "notify"
          ...
          if admin.notice_workflow_email_user_setting == "notify"
