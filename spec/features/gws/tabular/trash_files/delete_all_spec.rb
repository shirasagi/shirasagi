...
describe Gws::Tabular::TrashFilesController, type: :feature, dbscope: :example, js: true do
  ...
  shared_examples "common delete all spec" do
    it do
      ...
      model = Gws::Tabular::File[form.current_release]
      expect { model.find_by("col_#{column1.id}" => file1_name) }.to raise_error Mongoid::Errors::DocumentNotFound
      expect { model.find_by("col_#{column1.id}" => file2_name) }.to raise_error Mongoid::Errors::DocumentNotFound
    end
  end
  ...
  context "without any views" do
    ...
    include_examples "common delete all spec"
  end

  context "with list view" do
    let!(:list_view) do
      create(
        :gws_tabular_view_list, cur_site: site, cur_space: space, cur_form: form, state: 'public',
        title_column_ids: [ column1.id ])
    end
    let(:view) { list_view }
  ...
  context "with liquid view" do
    let!(:liquid_view) do
      create(
        :gws_tabular_view_liquid, cur_site: site, cur_space: space, cur_form: form, state: 'public')
    end
