...
describe Gws::Tabular::Gws::ColumnsController, type: :feature, dbscope: :example, js: true do
  ...
  let!(:form_referee1) do
    create :gws_tabular_form, cur_site: site, cur_space: space, state: 'closed', workflow_state: 'disabled'
  end
  let!(:form_referee2) do
    create :gws_tabular_form, cur_site: site, cur_space: space, state: 'closed', workflow_state: 'disabled'
  end
  let!(:form_referee3) do
    create :gws_tabular_form, cur_site: site, cur_space: space, state: 'closed', workflow_state: 'disabled'
  end

  context "with gws/tabular/column/reference_field" do
    let(:name1) { "name-#{unique_id}" }
    let(:reference_type1) { %w(one_to_one one_to_many).sample }
    let(:reference_type_label1) { I18n.t("gws/tabular.options.reference_type.#{reference_type1}") }
    ...
    let(:reference_type2) { %w(one_to_one one_to_many).sample }
    let(:reference_type_label2) { I18n.t("gws/tabular.options.reference_type.#{reference_type2}") }
    ...
    let(:reference_type3) { %w(one_to_one one_to_many).sample }
    let(:reference_type_label3) { I18n.t("gws/tabular.options.reference_type.#{reference_type3}") }
    ...
    it do
      ...
      within ".gws-column-list-toolbar[data-placement='top']" do
        wait_for_event_fired("gws:column:added") { click_on I18n.t("mongoid.models.gws/tabular/column/reference_field") }
      end
      ...
      within first(".gws-column-item") do
        ...
        select form_referee1.i18n_name, from: "item[reference_form_id]"
        within ".gws-column-form-input-group[data-field-name='reference_type']" do
          choose reference_type_label1
        end
        # オプション設定
      ...
      expect(column).to be_a(Gws::Tabular::Column::ReferenceField)
      expect(column.name).to eq name1
      expect(column.reference_form_id).to eq form_referee1.id
      expect(column.reference_type).to eq reference_type1
      ...
      within first(".gws-column-item") do
        ...
        select form_referee2.i18n_name, from: "item[reference_form_id]"
        within ".gws-column-form-input-group[data-field-name='reference_type']" do
          choose reference_type_label2
        end
        # オプション設定
        within ".gws-column-form-input-group[data-field-name='required']" do
      ...
      expect(column.reference_form_id).to eq form_referee2.id
      expect(column.reference_type).to eq reference_type2
      ...
      within_dialog do
        within "form#item-form" do
          ...
          # reference_field
          select form_referee3.i18n_name, from: "item[reference_form_id]"
          select reference_type_label3, from: "item[reference_type]"
          # tabular common
      ...
      # reference_field
      expect(column.reference_form_id).to eq form_referee3.id
      expect(column.reference_type).to eq reference_type3
      # tabular common
