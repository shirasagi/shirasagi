...
describe Gws::Tabular::Gws::ColumnsController, type: :feature, dbscope: :example, js: true do
  ...
  context "with gws/tabular/column/enum_field" do
    ...
    let(:select_options1) { Array.new(rand(1..5)) { "option-#{unique_id}" } }
    let(:input_type1) { %w(radio checkbox select).sample }
    let(:input_type_label1) { I18n.t("gws/tabular.options.enum_input_type.#{input_type1}") }
    ...
    let(:select_options2) { Array.new(rand(1..5)) { "option-#{unique_id}" } }
    let(:input_type2) { %w(radio checkbox select).sample }
    let(:input_type_label2) { I18n.t("gws/tabular.options.enum_input_type.#{input_type2}") }
    ...
    let(:select_options3) { Array.new(rand(1..5)) { "option-#{unique_id}" } }
    let(:input_type3) { %w(radio checkbox select).sample }
    let(:input_type_label3) { I18n.t("gws/tabular.options.enum_input_type.#{input_type3}") }
    ...
    it do
      ...
      within ".gws-column-list-toolbar[data-placement='top']" do
        wait_for_event_fired("gws:column:added") { click_on I18n.t("mongoid.models.gws/tabular/column/enum_field") }
      end
      ...
      within first(".gws-column-item") do
        ...
        fill_in "item[select_options]", with: select_options1.join("\n")
      ...
      expect(column).to be_a(Gws::Tabular::Column::EnumField)
      expect(column.name).to eq name1
      expect(column.select_options).to eq select_options1
      ...
      within first(".gws-column-item") do
        ...
        fill_in "item[select_options]", with: select_options2.join("\n")
        within ".gws-column-form-input-group[data-field-name='input_type']" do
      ...
      expect(column.select_options).to eq select_options2
      ...
      within_dialog do
        within "form#item-form" do
          ...
          # enum_field
          fill_in "item[select_options]", with: select_options3.join("\n")
      ...
      # enum_field
      expect(column.select_options).to eq select_options3
