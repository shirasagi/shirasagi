...
describe Gws::Tabular::Gws::ColumnsController, type: :feature, dbscope: :example, js: true do
  ...
  context "with gws/tabular/column/number_field" do
    let(:name1) { "name-#{unique_id}" }
    let(:field_type1) { %w(integer float decimal).sample }
    let(:field_type_label1) { I18n.t("gws/tabular.options.number_field_type.#{field_type1}") }
    let(:min_value1) { [ true, false ].sample ? rand(0..500) : nil }
    let(:max_value1) { [ true, false ].sample ? rand(0..500) + (min_value1 || 0) : nil }
    let(:default_value1) { [ true, false ].sample ? rand((min_value1 || 0)..(max_value1 || 500)) : nil }
    ...
    let(:field_type2) { %w(integer float decimal).sample }
    let(:field_type_label2) { I18n.t("gws/tabular.options.number_field_type.#{field_type2}") }
    let(:min_value2) { [ true, false ].sample ? rand(0..500) : nil }
    let(:max_value2) { [ true, false ].sample ? rand(0..500) + (min_value2 || 0) : nil }
    let(:default_value2) { [ true, false ].sample ? rand((min_value2 || 0)..(max_value2 || 500)) : nil }
    ...
    let(:max_value3) { [ true, false ].sample ? rand(0..500) + (min_value3 || 0) : nil }
    let(:default_value3) { [ true, false ].sample ? rand((min_value3 || 0)..(max_value3 || 500)) : nil }
    ...
    it do
      ...
      within ".gws-column-list-toolbar[data-placement='top']" do
        wait_for_event_fired("gws:column:added") { click_on I18n.t("mongoid.models.gws/tabular/column/number_field") }
      end
      ...
      within first(".gws-column-item") do
        ...
        within ".gws-column-form-input-group[data-field-name='field_type']" do
          choose field_type_label1
        end
        # 数値型
        fill_in "item[min_value]", with: min_value1
        fill_in "item[max_value]", with: max_value1
        fill_in "item[default_value]", with: default_value1
        # オプション設定
      ...
      expect(column).to be_a(Gws::Tabular::Column::NumberField)
      expect(column.name).to eq name1
      expect(column.field_type).to eq field_type1
      expect(column.min_value).to eq min_value1
      expect(column.max_value).to eq max_value1
      expect(column.default_value).to eq default_value1
      ...
      within first(".gws-column-item") do
        ...
        within ".gws-column-form-input-group[data-field-name='field_type']" do
          choose field_type_label2
        end
        # 数値型
        fill_in "item[min_value]", with: min_value2
        fill_in "item[max_value]", with: max_value2
        fill_in "item[default_value]", with: default_value2
        # オプション設定
        within ".gws-column-form-input-group[data-field-name='required']" do
      ...
      expect(column.field_type).to eq field_type2
      expect(column.min_value).to eq min_value2
      expect(column.max_value).to eq max_value2
      expect(column.default_value).to eq default_value2
      ...
      within_dialog do
        within "form#item-form" do
          ...
          fill_in "item[max_value]", with: max_value3
          fill_in "item[default_value]", with: default_value3
          # tabular common
      ...
      expect(column.max_value).to eq max_value3
      expect(column.default_value).to eq default_value3
      # tabular common
