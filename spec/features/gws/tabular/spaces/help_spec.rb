...
describe Gws::Tabular::SpacesController, type: :feature, dbscope: :example, js: true do
  ...
  context "list head manual (汎用DB全体のヘルプ)" do
    ...
    it "toggles the popup open and closed by clicking the help icon" do
      login_user admin, to: gws_tabular_spaces_path(site: site)
      wait_for_js_ready # SS_DropdownToggle のバインド完了を待つ
      within ".gws-tabular-space-help" do
        # 既定は閉じている（active なし）
        expect(page).to have_no_css(".gws-menu-help-popup.active")
        # クリックで開く
        find(".gws-menu-help__icon").click
        expect(page).to have_css(".gws-menu-help-popup.active")
        # 再クリックで閉じる
        find(".gws-menu-help__icon").click
        expect(page).to have_no_css(".gws-menu-help-popup.active")
      end
