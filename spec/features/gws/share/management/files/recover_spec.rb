...
describe "gws_share_files", type: :feature, dbscope: :example, js: true do
  ...
  describe "restore file whose original folder was deleted" do
    let!(:folder2) { create :gws_share_folder, cur_site: site }

    before do
      folder.delete # ファイルが存在するフォルダーは通常削除できないが、無理やり削除する
    ...
    it do
      ...
      visit gws_share_files_path(site: site)
      within ".tree-navi" do
        expect(page).to have_css(".item-name", text: folder2.name)
        expect(page).to have_no_css(".item-name", text: folder.name)
      end
      ...
      wait_for_error I18n.t("activemodel.errors.models.gws/share/recover_file_service.not_found_parent")
