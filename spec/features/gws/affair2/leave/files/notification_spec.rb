...
describe "gws_affair2_leave_files", type: :feature, dbscope: :example, js: true do
  let!(:site) { gws_site }
  ...
  def create_leave_file
    visit new_gws_affair2_leave_file_path(site)

    within "form#item-form" do
      fill_in "item[name]", with: unique_id
      within ".date-field" do
        fill_in_datetime "item[in_start_date]", with: "2025/1/6"
        fill_in_datetime "item[in_close_date]", with: "2025/1/6"
      end
      select I18n.t("gws/affair2.options.leave_type.sick1"), from: "item[leave_type]"
      click_button I18n.t('ss.buttons.save')
    end
    wait_for_notice I18n.t("ss.notice.saved")
    Gws::Affair2::Leave::File.first
  end
  ...
  context "regular user" do
    ...
    it do
      Timecop.travel(month) do
        ...
        expect(notification.subject).to eq I18n.t("gws_notification.gws/affair2/leave/file.request", name: item.name)
        ...
        expect(notification.subject).to eq I18n.t("gws_notification.gws/affair2/leave/file.approve", name: item.name)
        expect(notification.url).to eq item.private_show_path
      end
    end
  end
  ...
  context "regular user" do
    ...
    it do
      Timecop.travel(month) do
        ...
        expect(SS::Notification.member(user2).count).to eq 1
        notification = SS::Notification.member(user1).first
        expect(notification.subject).to eq I18n.t("gws_notification.gws/affair2/leave/file.remand", name: item.name)
        expect(notification.url).to eq item.private_show_path
      end
    end
  end
  ...
  context "regular user" do
    ...
    it do
      Timecop.travel(month) do
        ...
        expect(notification.subject).to eq I18n.t("gws_notification.gws/affair2/leave/file.circular", name: item.name)
        expect(notification.url).to eq item.private_show_path
        ...
        notification = SS::Notification.member(user1).first
        expect(notification.subject).to eq I18n.t("gws_notification.gws/affair2/leave/file.comment", name: item.name)
        expect(notification.url).to eq item.private_show_path
      end
    end
  end
