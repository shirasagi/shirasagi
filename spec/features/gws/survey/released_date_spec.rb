...
describe "gws_survey", type: :feature, dbscope: :example, js: true do
  ...
  context "with reserved(unreleased) public form" do
    let!(:item1) do
      create(
        :gws_survey_form, state: "public", release_date: now + 1.day, due_date: now + 2.days,
    ...
    it do
      ...
      Gws::Survey::Form.find(item1.id).tap do |item|
        ...
        expect(item.release_date).to eq now + 1.day
