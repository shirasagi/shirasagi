...
describe "article_pages" do
  subject(:site) { cms_site }
  subject(:node) { create_once :article_node_page, filename: "docs", name: "article" }
  subject(:item) { Article::Page.last }
  subject(:index_path) { article_pages_path site.host, node }
  subject(:new_path) { new_article_page_path site.host, node }
  subject(:show_path) { article_page_path site.host, node, item }
  subject(:edit_path) { edit_article_page_path site.host, node, item }
  subject(:delete_path) { delete_article_page_path site.host, node, item }
  subject(:move_path) { move_article_page_path site.host, node, item }
  subject(:copy_path) { copy_article_page_path site.host, node, item }
