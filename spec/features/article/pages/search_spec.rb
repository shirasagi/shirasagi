...
describe "article_pages", type: :feature, dbscope: :example, js: true do
  ...
  let(:item1) { create(:article_page, name: "item1", cur_node: node, released: released) }
  let(:item2) { create(:article_page, name: "item2", cur_node: node, released: released.advance(days: 1)) }
  let(:item3) { create(:article_page, name: "item3", cur_node: node, released: released.advance(days: 2)) }
  let(:item4) { create(:article_page, name: "item4", cur_node: node, released: released.advance(days: 3)) }
  let(:item5) { create(:article_page, name: "item5", cur_node: node, released: released.advance(days: 4)) }
  let(:item6) { create(:article_page, name: "item6", cur_node: node, state: "closed") }
