...
describe "article_pages", type: :feature, dbscope: :example, js: true do
  ...
  context "when assoc_job mode is realtime" do
    ...
    around do |example|
      ...
      SS.config.replace_value_at(:opendata, :assoc_job, @save_config)
    end
  ...
  context "when assoc_job mode is skip" do
    ...
    it 'public' do
      ...
      expect(Opendata::Dataset.site(od_site).count).to eq 0
  ...
  context "when assoc_job mode is blank" do
    ...
    around do |example|
      @save_config = SS.config.replace_value_at(:opendata, :assoc_job, { "perform" => 'later' })
      perform_enqueued_jobs do
        example.run
      end
      SS.config.replace_value_at(:opendata, :assoc_job, @save_config)
