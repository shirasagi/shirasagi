...
describe "opendata_agents_nodes_my_app", type: :feature, dbscope: :example, js: true do
  ...
  let!(:upper_html) { "<a href=\"new/\">#{I18n.t("ss.links.new")}</a><table class=\"opendata-app apps\"><tbody>" }
  let!(:node_myapp) do
  ...
  describe "basic crud" do
    let!(:node_search_dateset) { create :opendata_node_search_dataset }
    21.times do |i|
  ...
  describe "new error" do
    it do
      ...
      expect(page).to have_css('#errorExplanation', text: I18n.t("errors.template.header", count: 1))
    end
  end
