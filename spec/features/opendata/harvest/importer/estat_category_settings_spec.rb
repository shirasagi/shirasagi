...
describe "opendata_harvest_importer", dbscope: :example, js: true do
  ...
  context "basic crud" do
    ...
    it "#show" do
      visit show_path
      first("#addon-opendata-agents-addons-harvest-importer_estat_category_setting").click
      within "#addon-opendata-agents-addons-harvest-importer_estat_category_setting" do
        expect(page).to have_link estat_category1.name
        expect(page).to have_link estat_category2.name
        expect(page).to have_link estat_category3.name
        click_on estat_category1.name
      end
