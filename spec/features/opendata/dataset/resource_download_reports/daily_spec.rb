...
describe Opendata::Dataset::ResourceDownloadReportsController, type: :feature, dbscope: :example do
  ...
  let!(:report2) do
    create(
      :opendata_resource_download_report, cur_site: site, deleted: now, dataset_id: rand(30..40), resource_id: rand(30..40)
    )
  end
  let!(:report3) do
    create(
  ...
  describe "#download" do
    it do
      ...
      expect(table.headers[6]).to eq "#{now.year}#{I18n.t('datetime.prompts.year')}#{now.month}#{I18n.t('datetime.prompts.month')}"
