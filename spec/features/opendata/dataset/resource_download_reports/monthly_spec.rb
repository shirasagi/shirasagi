...
describe Opendata::Dataset::ResourceDownloadReportsController, type: :feature, dbscope: :example do
  ...
  let!(:report1) do
    create(:opendata_resource_download_report, cur_site: site, dataset_id: rand(10..20), resource_id: rand(10..20))
  end
  let!(:report2) do
    create(
      :opendata_resource_download_report, cur_site: site, deleted: Opendata::ResourceDownloadReport::UNCERTAIN_DELETED_TIME,
      dataset_id: rand(30..40), resource_id: rand(30..40)
    )
  end
  let!(:report3) do
    create(
      :opendata_resource_download_report, cur_site: site, year_month: last_year.year * 100 + last_year.month,
      dataset_id: rand(30..40), resource_id: rand(30..40)
    )
  end
  ...
  describe "#index" do
    it do
      visit opendata_dataset_report_main_path(site: site, cid: node)
      within "form.search" do
        select I18n.t("activemodel.attributes.opendata/dataset_download_report/type.month"), from: "s[type]"
  ...
  describe "#download" do
    it do
      ...
      expect(table.headers[6]).to eq "#{now.year}#{I18n.t('datetime.prompts.year')}"
      expect(table.headers.length).to eq 19
      ...
      expect(table[3][Opendata::Dataset.t("state")]).to eq I18n.t("ss.options.state.deleted")
    end
  end
end
