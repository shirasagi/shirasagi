...
describe "inquiry_site_answers", type: :feature, dbscope: :example, js: true do
  let!(:site) { cms_site }
  let!(:user) { cms_user }
  ...
  let(:answer) do
    Inquiry::Answer.new(
      cur_site: site, cur_node: node, remote_addr: remote_addr, user_agent: user_agent, group_ids: user.group_ids)
  end
  ...
  before do
    ...
    site.update!(inquiry_form: node)
  end

  context "basic crud with answer admin" do
    before { login_user user, to: cms_contents_path(site: site) }
