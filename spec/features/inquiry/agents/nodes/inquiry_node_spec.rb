...
describe "inquiry_agents_nodes_node", type: :feature, dbscope: :example do
  let(:site) { cms_site }
  ...
  context "liquid" do
    ...
    it do
      ...
      expect(page).to have_css("h2 a", text: child_node1.name)
      expect(page).to have_css("h2 a", text: child_node2.name)
      expect(page).to have_css("h2 a", text: child_node3.name)
