...
describe "webmail_mails", type: :feature, dbscope: :example, imap: true, js: true do
  context "when mail is sent with eml file" do
    ...
    let(:content) { Rails.root.join("spec/fixtures/webmail/mail-1.eml") }
    let!(:file) do
      tmp_ss_file(contents: content, user: user, content_type: "message/rfc822")
    end

    shared_examples "webmail/mails send with eml attachment flow" do
      before do
    ...
    shared_examples "webmail/mails account and group flow" do
      ...
      describe "webmail_mode is account" do
        ...
        it_behaves_like "webmail/mails send with eml attachment flow"
      end
