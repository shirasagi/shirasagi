...
describe "webmail_group_group_addresses", type: :feature, dbscope: :example do
  let(:group) { create :webmail_group }
  let(:address_group) { create :webmail_address_group }
  let!(:item) { create :webmail_address, address_group_id: address_group.id }
  let(:index_path) { webmail_address_group_group_addresses_path(group: group, address_group: address_group.id) }
