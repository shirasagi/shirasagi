...
describe "webmail_users", type: :feature, dbscope: :example, js: true do
  let(:user) { webmail_admin }

  let!(:user1) { create :webmail_user, name: "user1", uid: "user1", email: "user1@example.jp" }
  let!(:user2) { create :webmail_user, name: "user2", uid: "user2", email: "user2@example.jp", deletion_lock_state: "locked" }
  let(:index_path) { webmail_users_path }
  let(:delete_user1_path) { delete_webmail_user_path user1 }
  let(:delete_user2_path) { delete_webmail_user_path user2 }
