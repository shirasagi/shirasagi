...
describe Opendata::Harvest::ImportJob, dbscope: :example, tmpdir: true, ckan: true do
  ...
  let(:dataset_params) do
    { name: unique_id, title: unique_id, notes: unique_id, owner_org: "org1", license_id: license.uid }
  end
  ...
  it do
    ...
    # add ckan dataset
    ckan_package.package_create(dataset_params, api_key)
    expect(ckan_package.package_list).to match_array [dataset1.uuid, dataset2.uuid, dataset_params[:name]]

    # import datasets
    Opendata::Harvest::ImportJob.bind(site_id: site.id).perform_now(exporter_id: exporter.id)
    expect(Opendata::Dataset.site(site).where(harvest_importer_id: importer.id).count).to eq 3

    imported_dataset3 = Opendata::Dataset.site(site).where(name: dataset_params[:title]).first
    expect(imported_dataset3).not_to eq nil
    expect(imported_dataset3.text).to eq dataset_params[:notes]
    expect(imported_dataset3.resources.size).to eq 0
  end
end
