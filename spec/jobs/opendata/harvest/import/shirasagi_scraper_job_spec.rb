...
describe Opendata::Harvest::ImportJob, type: :feature, dbscope: :example do
  let!(:site) { cms_site }
  ...
  context "with empty datasets" do
    ...
    describe "#perform" do
      before do
        ...
        expect { job.perform_now }.to output(include("dataset_urls 0\n")).to_stdout
      end
  ...
  context "with datasets html" do
    ...
    describe "#perform" do
      before do
        ...
        job = described_class.bind(site_id: site.id)
        expect { job.perform_now }.to output(include("dataset_urls 5\n")).to_stdout
      end
