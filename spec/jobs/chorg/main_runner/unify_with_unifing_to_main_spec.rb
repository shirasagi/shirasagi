...
describe Chorg::MainRunner, dbscope: :example do
  ...
  context "unify with unifing contacts to main" do
    ...
    it do
      ...
      Cms::Group.find(source_group1.id).tap do |group|
        ...
        expect(group.contact_charge).to eq destination_contact[:contact_charge]
        ...
        expect(group.contact_postal_code).to eq destination_contact[:contact_postal_code]
        expect(group.contact_address).to eq destination_contact[:contact_address]
        ...
        expect(group.updated.in_time_zone).to eq source_group1.updated.in_time_zone
        ...
        group.contact_groups.first.tap do |new_contact|
          ...
          expect(new_contact.contact_charge).to eq destination_contact[:contact_charge]
          ...
          expect(new_contact.contact_postal_code).to eq destination_contact[:contact_postal_code]
          expect(new_contact.contact_address).to eq destination_contact[:contact_address]
      ...
      Article::Page.find(article_page1.id).tap do |page|
        ...
        expect(page.contact_group_name).to eq new_contact.contact_group_name
        expect(page.contact_charge).to eq new_contact.contact_charge
        ...
        expect(page.contact_postal_code).to eq new_contact.contact_postal_code
        expect(page.contact_address).to eq new_contact.contact_address
  ...
  context "unify to existing group with unifing contacts to main" do
    let!(:source_group1) { Timecop.freeze(now - 4.weeks) { create(:revision_new_group, order: 10) } }
    let!(:source_group2) { Timecop.freeze(now - 5.weeks) { create(:revision_new_group, order: 20) } }
    let!(:destination_group) { Timecop.freeze(now - 6.weeks) { create(:revision_new_group, order: 30) } }
    ...
    let!(:article_page2) do
      ...
      Timecop.freeze(now - 3.weeks) do
        page = create(
          :article_page, cur_site: site, cur_node: article_node, group_ids: [ source_group1.id, source_group2.id ],
          contact_group_id: source_group1.id, contact_group_contact_id: contact.id, contact_group_relation: "related",
          contact_group_name: contact.contact_group_name, contact_charge: contact.contact_charge, contact_tel: contact.contact_tel,
    ...
    it do
      ...
      Cms::Group.find(destination_group.id).tap do |group|
        ...
        expect(group.contact_link_url).to eq destination_contact[:contact_link_url]
        expect(group.contact_link_name).to eq destination_contact[:contact_link_name]
        expect(group.updated.in_time_zone).to eq destination_group.updated.in_time_zone
        expect(group.contact_groups.count).to eq 1
        group.contact_groups.first.tap do |new_contact|
          expect(new_contact.main_state).to eq "main"
          expect(new_contact.main_state).to eq destination_contact[:main_state]
