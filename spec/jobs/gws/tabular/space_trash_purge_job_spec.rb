...
describe Gws::Tabular::SpaceTrashPurgeJob, dbscope: :example do
  let!(:site) { gws_site }
  let!(:admin) { gws_user }
  ...
  before do
    ...
    space.update!(state: "closed", deleted: space_deleted_at)
  end
  ...
  describe '#perform' do
    ...
    context 'space is purged' do
      let(:space_deleted_at) { now - 2.years }

      it do
        expect { described_class.bind(site_id: site.id).perform_now }.to change { Gws::Tabular::Space.all.count }.by(-1)
        ...
        expect(Gws::Tabular::Space.all.count).to eq 0
        expect(Gws::Tabular::Form.all.count).to eq 0
    ...
    context 'no spaces are purged' do
      let(:space_deleted_at) { now - 7.days }
      ...
      it do
        ...
        expect(Gws::Tabular::Space.all.count).to be > 0
        expect(Gws::Tabular::Form.all.count).to be > 0
    ...
    context 'group trash_threshold is 7.days => space is purged' do
      let(:space_deleted_at) { now - 7.days }
    ...
    context 'group trash_threshold is 3.years => no spaces are purged' do
      let(:space_deleted_at) { now - 7.days }
