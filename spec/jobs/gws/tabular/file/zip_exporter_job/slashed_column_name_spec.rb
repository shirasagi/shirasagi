        ...
        I18n.with_locale(I18n.default_locale) do
          ...
          SS::Csv.open(StringIO.new(csv), headers: true) do |csv|
            ...
            csv_table[0].tap do |csv_row|
              ...
              expect(csv_row[SS::Csv.escape_column_name_for_csv(column2.name)]).to eq "#{file_data1.id}/#{attachment1_1.id}_#{attachment1_1.filename}"
              expect(csv_row[SS::Csv.escape_column_name_for_csv(column3.name)]).to eq "#{file_data1.id}/#{attachment1_2.id}_#{attachment1_2.filename}"
            end
            csv_table[1].tap do |csv_row|
              ...
              expect(csv_row[column2.name.gsub('/', '\/')]).to eq "#{file_data2.id}/#{attachment2_1.id}_#{attachment2_1.filename}"
              expect(csv_row[column3.name.gsub('/', '\/')]).to eq "#{file_data2.id}/#{attachment2_2.id}_#{attachment2_2.filename}"
            end
          end
        end
      end
