...
describe Gws::Schedule::TodoTrashPurgeJob, dbscope: :example do
  ...
  let!(:item1) { create(:gws_schedule_todo, cur_site: site, deleted: now - 7.days) }
  let!(:item2) { create(:gws_schedule_todo, cur_site: site, deleted: now - 2.years) }

  describe '#perform' do
    context '1 todos are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Schedule::Todo.count }.by(-1)
    ...
    context 'no todos are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now(threshold: '3.years') }.to \
          change { Gws::Schedule::Todo.count }.by(0)
    ...
    context 'when group trash_threshold is 7.days' do
      ...
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Schedule::Todo.count }.by(-2)
