...
describe Gws::Schedule::TrashPurgeJob, dbscope: :example do
  ...
  let!(:item1) { create(:gws_schedule_plan, cur_site: site, deleted: now - 7.days) }
  let!(:item2) { create(:gws_schedule_plan, cur_site: site, deleted: now - 2.years) }

  describe '#perform' do
    context '1 plans are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Schedule::Plan.count }.by(-1)
    ...
    context 'no plans are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now(threshold: '3.years') }.to \
          change { Gws::Schedule::Plan.count }.by(0)
