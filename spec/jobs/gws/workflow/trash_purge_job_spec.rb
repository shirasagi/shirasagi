...
describe Gws::Workflow::TrashPurgeJob, dbscope: :example do
  ...
  let!(:item1) { create(:gws_workflow_file, cur_site: site, deleted: now - 7.days) }
  let!(:item2) { create(:gws_workflow_file, cur_site: site, deleted: now - 2.years) }
  ...
  describe '#perform' do
    ...
    context '2 workflow files are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now(threshold: '7.days') }.to \
          change { Gws::Workflow::File.count }.by(-2)
    ...
    context 'no workflow files are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now(threshold: '3.years') }.to \
          change { Gws::Workflow::File.count }.by(0)
    ...
    context 'when group trash_threshold is 7.days' do
      ...
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Workflow::File.count }.by(-2)
    ...
    context 'when group trash_threshold is 3.years' do
      ...
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Workflow::File.count }.by(0)
