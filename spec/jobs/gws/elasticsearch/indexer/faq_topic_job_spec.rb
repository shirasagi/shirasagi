      ...
      it do
        ...
        site.elasticsearch_client.search(index: "g#{site.id}", size: 100, q: "*:*").tap do |es_docs|
          ...
          es_docs["hits"]["hits"][0].tap do |es_doc|
            expect(es_doc["_id"]).to eq "gws_faq_posts-post-#{topic.id}"
            source = es_doc["_source"]
            expect(source['url']).to eq "/.g#{site.id}/faq/-/-/topics/#{topic.id}#post-#{topic.id}"
          end
          es_docs["hits"]["hits"][1].tap do |es_doc|
            expect(es_doc["_id"]).to eq "file-#{file.id}"
            source = es_doc["_source"]
            expect(source['url']).to eq "/.g#{site.id}/faq/-/-/topics/#{topic.id}#file-#{file.id}"
          end
        end
      end
    end
