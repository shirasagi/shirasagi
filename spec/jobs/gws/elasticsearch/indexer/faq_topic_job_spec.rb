...
describe Gws::Elasticsearch::Indexer::FaqTopicJob, dbscope: :example, es: true do
  ...
  let(:category) { create(:gws_faq_category, cur_site: site) }
      ...
      it do
        ...
        perform_enqueued_jobs do
          expectation = expect do
            topic = create(:gws_faq_topic, cur_site: site, cur_user: user, category_ids: [category.id], file_ids: [file.id])
          end
        ...
        es_requests.first.tap do |request|
          ...
          expect(request['uri']['path']).to end_with("/gws_faq_posts-post-#{topic.id}")
          body = JSON.parse(request['body'])
          expect(body['url']).to eq "/.g#{site.id}/faq/-/-/topics/#{topic.id}#post-#{topic.id}"
        end
        es_requests.second.tap do |request|
          expect(request['method']).to eq 'put'
          expect(request['uri']['path']).to end_with("/file-#{file.id}")
          body = JSON.parse(request['body'])
          expect(body['url']).to eq "/.g#{site.id}/faq/-/-/topics/#{topic.id}#file-#{file.id}"
        end
      end
    end
