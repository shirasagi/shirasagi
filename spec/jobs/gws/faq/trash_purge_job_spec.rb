...
describe Gws::Faq::TrashPurgeJob, dbscope: :example do
  ...
  let!(:item1) { create(:gws_faq_topic, cur_site: site, deleted: now - 7.days) }
  let!(:item2) { create(:gws_faq_topic, cur_site: site, deleted: now - 2.years) }

  describe '#perform' do
    context '1 faq topics are purged' do
      it do
    ...
    context 'no faq topics are purged' do
      it do
        expect { described_class.bind(site_id: site).perform_now(threshold: '3.years') }.to \
          change { Gws::Faq::Topic.topic.count }.by(0)
    ...
    context 'when group trash_threshold is 7.days' do
      ...
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Faq::Topic.topic.count }.by(-2)
    ...
    context 'when group trash_threshold is 3.years' do
      ...
      it do
        expect { described_class.bind(site_id: site).perform_now }.to change { Gws::Faq::Topic.topic.count }.by(0)
