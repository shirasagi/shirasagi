...
describe Cms::Node::CopyNodesJob, dbscope: :example do
  describe "copy node" do
    ...
    let!(:task) do
      create(
        :copy_nodes_task, site_id: site.id, node_id: node1.id, target_node_name: target_node_name
      )
    end
    ...
    it do
      expect do
        ...
        job.perform_now(target_node_name: target_node_name)
      ...
      copied_node = Cms::Node.site(site).where(filename: target_node_name).first
      expect(copied_node).to be_a(Inquiry::Node::Form)
      expect(copied_node.site_id).to eq site.id
      expect(copied_node.name).to eq node1.name
