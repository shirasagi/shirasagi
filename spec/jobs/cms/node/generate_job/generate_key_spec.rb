...
describe Cms::Node::GenerateJob, dbscope: :example do
  ...
  let(:web01_expected_pages) { Cms::Page.where(depth: 1).select { |item| (item.id % generate_keys.size) == 0 } }
  let(:web02_expected_pages) { Cms::Page.where(depth: 1).select { |item| (item.id % generate_keys.size) == 1 } }
  let(:web03_expected_pages) { Cms::Page.where(depth: 1).select { |item| (item.id % generate_keys.size) == 2 } }

  let(:web01_expected_nodes) { Cms::Node.all.select { |item| (item.id % generate_keys.size) == 0 } }
  let(:web02_expected_nodes) { Cms::Node.all.select { |item| (item.id % generate_keys.size) == 1 } }
  let(:web03_expected_nodes) { Cms::Node.all.select { |item| (item.id % generate_keys.size) == 2 } }
