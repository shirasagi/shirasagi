namespace :opendata do
  ...
  task pivot: :environment do
    ...
    def extract_pivot(columns)
      ...
      if column_range
        return [line_range, column_range]
      else
        return nil
      end
    end

    def extract_columns(csv_array)
      columns = []
    ...
    def format_csv_array(csv_array)
    ...
    Opendata::Dataset.each do |item|
      item.resources.each do |r|
        next if r.format != "CSV"

        begin
          encoding = NKF.guess(Fs.read(r.file.path))
          if encoding.name.downcase =~ /shift_jis|windows/
            encoding = "cp932"
          end

          csv_array = ::CSV.open(r.file.path, encoding: "#{encoding}:UTF-8").to_a
          csv_array = format_csv_array(csv_array)
          columns = extract_columns(csv_array)
          pivot = extract_pivot(columns)

          if pivot
            puts "#{r.id} #{pivot}"
            extracted_csv = csv_array[pivot[1]].map do |item|
              item[pivot[0]]
            end

            csv_data = CSV.generate do |data|
              extracted_csv.each do |item|
                data << item
              end
            end
            csv_data = "\uFEFF".freeze + csv_data

            Fs.mkdir_p("csv")
            ::Fs.write("csv/#{item.name}_#{r.name}_#{r.id}.csv", csv_data)
          end
        rescue => e
          puts "error : #{item.id} #{item.name} #{r.id}:#{r.path} :  #{e.class} (#{e.message})"
        end
      end
    end
  end
