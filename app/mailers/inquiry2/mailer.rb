class Inquiry2::Mailer < ApplicationMailer
  helper Inquiry2::MailerHelper
  ...
  def notify_mail(site, node, answer, notice_email)
    ...
    @answer_url = ::File.join(
      ...
      Rails.application.routes.url_helpers.inquiry2_answer_path(site: site.id, cid: node.id, id: answer.id)
    )
    @subject = "[#{I18n.t('inquiry2.notify')}]#{node.name} - #{site.name}"
