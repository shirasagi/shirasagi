class Workflow::PagesController < ApplicationController
  ...
  def request_approval
    ...
    current_workflow_approvers.each do |workflow_approver|
      ...
      Workflow::Mailer.request_mail(args).deliver_now if @cur_user.email_enabled?(args[:t_uid])
    end
  ...
  def workflow_alert
    email_blank_users = []
    target_user = nil
    email_blank_users.push(@cur_user.name) if @cur_user.email_disabled?
    if params[:workflow_approvers].present?
      params[:workflow_approvers].each do |workflow_approver|
        ...
        if target_user
          email_blank_users.push(target_user.name) if target_user.email_disabled?
        end
      end
    else
      current_level = @item.workflow_current_level
  ...
  def approve_update
    ...
    if @item.update
      ...
      if workflow_state == @model::WORKFLOW_STATE_APPROVE
        ...
        Workflow::Mailer.approve_mail(args).deliver_now if @cur_user.email_enabled?(args[:t_uid])
    ...
    if @item.update
      if @item.workflow_state == "remand"
        args = { f_uid: @cur_user._id, t_uid: @item.workflow_user_id,
                 site: @cur_site, page: @item,
                 url: params[:url], comment: params[:remand_comment] }
        Workflow::Mailer.remand_mail(args).deliver_now if @cur_user.email_enabled?(args[:t_uid])
      end
      render json: { workflow_state: @item.workflow_state }
    else
      render json: @item.errors.full_messages, status: :unprocessable_entity
