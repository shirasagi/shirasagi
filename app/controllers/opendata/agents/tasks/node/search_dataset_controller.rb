class Opendata::Agents::Tasks::Node::SearchDatasetController < ApplicationController
  ...
  def generate_search_file(node)
    ...
    @model = node.class
    @cur_site = node.site
    @cur_node = Opendata::Node::SearchDataset.site(@cur_site).and_public.first
    ...
    file = node.dataset_search_html_path
    ...
    response.body = render partial: 'opendata/agents/nodes/dataset/search_dataset/search'
