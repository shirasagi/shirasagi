class Opendata::Agents::Pages::App::AppController < ApplicationController
  ...
  def index
    ...
    if @cur_page.dataset_ids.empty? == false
      @ds = Opendata::Dataset.site(@cur_site).and_public(@cur_date).in(id: @cur_page.dataset_ids)
    end

    @app_idea = Opendata::Idea.site(@cur_site).and_public(@cur_date).where(app_ids: @cur_page.id)
