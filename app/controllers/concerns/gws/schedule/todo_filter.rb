module Gws::Schedule::TodoFilter
  ...
  def item_readable?
    @item.member_include?(@cur_user) ||
      @item.readable?(@cur_user, site: @cur_site) ||
  ...
  def finish
    ...
    raise '403' if !@item.allowed?(:edit, @cur_user, site: @cur_site) && !@item.member_include?(@cur_user)
    @item.errors.clear
  ...
  def finish_all
    ...
    @items.each do |item|
      item.attributes = fix_params
      if item.allowed?(:edit, @cur_user, site: @cur_site) || item.member_include?(@cur_user)
        comment = Gws::Schedule::TodoComment.new(cur_site: @cur_site, cur_user: @cur_user, cur_todo: item)
        comment.achievement_rate = 100
        result = comment.save
