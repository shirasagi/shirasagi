module Sys::SiteCopyParts
  private
    ...
    def calc_cur_node(base_cmsnode, new_site)
    ...
    def set_cstm_attrs(base_cmsnode, new_site, node_fac_cats, new_cmsnode_attrs)
        # 施設の種類・用途・地域
        if base_cmsnode.route == "facility/page"
          new_cmsnode_attrs[:category_ids] = _pase_checkboxes_for_dupcms(
            node_fac_cats, "facility/category", base_cmsnode.category_ids
          )
          new_cmsnode_attrs[:service_ids] = _pase_checkboxes_for_dupcms(
            node_fac_cats, "facility/service", base_cmsnode.service_ids
          )
          new_cmsnode_attrs[:location_ids] = _pase_checkboxes_for_dupcms(
