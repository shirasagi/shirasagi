class Cms::Ldap::ImportsController < ApplicationController
  ...
  def sync
    ...
    if task.running?
      # already started
      redirect_to(cms_ldap_result_index_path, { notice: t("ldap.messages.sync_already_started") })
      return
    end

    task.results = nil
    task.save
    Cms::Ldap::SyncJob.bind(site_id: @cur_site, user_id: @cur_user).
      perform_later(@cur_site.root_group.id, @item.id)
    respond_to do |format|
      format.html { redirect_to(cms_ldap_result_index_path, { notice: t("ldap.messages.sync_started") }) }
      format.json { head :no_content }
    end
  end
end
