class Gws::Workflow2::Form::ApplicationsController < ApplicationController
  ...
  def copy
    ...
    if request.get? || request.head?
      translations = @item.i18n_name_translations
      SS.locales_in_order.each do |lang|
        if translations[lang].present?
          translations[lang] = "#{I18n.with_locale(lang) { t("gws/notice.prefix.copy") }} #{translations[lang]}"
        end
      end
      @item.i18n_name_translations = translations.dup
      render
      return
    end

    service = Gws::Column::CopyService.new(cur_site: @cur_site, cur_user: @cur_user, model: @model, item: @item)
    service.overwrites = params.require(:item).permit(:i18n_name, i18n_name_translations: I18n.available_locales)
