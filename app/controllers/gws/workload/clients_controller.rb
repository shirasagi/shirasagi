class Gws::Workload::ClientsController < ApplicationController
  include Gws::BaseFilter
  include Gws::CrudFilter
  include Gws::Workload::YearFilter

  model Gws::Workload::Client
  ...
  def set_crumbs
    ...
    @crumbs << [I18n.t('mongoid.models.gws/workload/client'), { action: :index }]
  end
  ...
  def download_all
    ...
    @item = Gws::Workload::Importer::Client.new(@cur_site, @year, @cur_user)
    enumerable = @item.enum_csv(encoding: encoding)
    filename = "gws_workload_clients_#{Time.zone.now.to_i}.csv"
