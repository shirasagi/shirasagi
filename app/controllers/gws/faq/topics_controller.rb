class Gws::Faq::TopicsController < ApplicationController
  ...
  def set_category
    @categories = Gws::Faq::Category.site(@cur_site).readable(@cur_user, @cur_site).tree_sort
    if category_id = params[:category].presence
      @category ||= Gws::Faq::Category.site(@cur_site).readable(@cur_user, @cur_site).where(id: category_id).first
    end
  end
