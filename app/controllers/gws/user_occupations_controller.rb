class Gws::UserOccupationsController < ApplicationController
  include Gws::BaseFilter
  include Gws::CrudFilter

  model Gws::UserOccupation
  ...
  def set_crumbs
    @crumbs << [t("mongoid.models.ss/user_occupation"), gws_user_occupations_path]
  end
  ...
  def download_all
    ...
    filename = "gws_user_occupations_#{Time.zone.now.to_i}.csv"
  ...
  def import
    ...
    unless Gws::UserOccupationImportJob.valid_csv?(@item.in_file)
    ...
    # call job
    Gws::UserOccupationImportJob.bind(site_id: @cur_site, user_id: @cur_user).perform_later(ss_file.id)
