class Gws::Affair2::Book::PaidLeaveController < ApplicationController
  ...
  model Gws::Affair2::Book::PaidLeave
  ...
  def set_crumbs
    ...
    @crumbs << [ t("modules.gws/affair2/book/paid_leave"), action: :index ]
  end

  def set_item
    @item = @model.new
    @item.load(@cur_site, @cur_user, @fiscal_year, @cur_group)
  end

  def set_cur_year
    raise '404' if params[:year].blank? || params[:year].length != 4
    @fiscal_year = params[:year].to_i
  end
