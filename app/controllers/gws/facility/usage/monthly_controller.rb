class Gws::Facility::Usage::MonthlyController < ApplicationController
  ...
  def set_crumbs
    ...
    @crumbs << [t('gws/facility.navi.usage'), gws_facility_usage_monthly_index_path]
  end
  ...
  def set_search_params
    ...
    @s ||= begin
      ...
      s[:month] ||= @target_time.month
      s
    end
  end

  def find_usage(facility, year, month, day)
    @aggregation.find do |data|
      data['_id']['facility_id'] == facility.id && data['_id']['month'] == month && data['_id']['day'] == day
    end
  end

  def format_usage_count(facility, year, month, day)
    data = find_usage(facility, year, month, day)
    return unless data
    count = data['count']
    count.to_s(:delimited)
  end

  def format_usage_hours(facility, year, month, day)
    data = find_usage(facility, year, month, day)
    return unless data
  ...
  def index
    ...
    @days = (sd..ed).to_a.map { |d| ["#{d}#{t('datetime.prompts.day')}", d] }
    ...
    criteria = criteria.gte(start_at: @target_time).lt(start_at: @target_time + 1.month)
