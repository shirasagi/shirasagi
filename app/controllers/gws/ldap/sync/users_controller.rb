class Gws::Ldap::Sync::UsersController < ApplicationController
  ...
  helper_method :dry_run_results?, :dry_run_users
  ...
  def show
    ...
    path = "#{@task.base_dir}/dry_run.zip"
    Zip::File.open(path) do |zip|
      entry_name = "collections/#{Gws::User.collection_name}/#{params[:id]}.json"
      entry = zip.find_entry(entry_name)
      raise "404" unless entry

      json = entry.get_input_stream.read
      json = JSON.parse(json)
      @after_item = ::Mongoid::Factory.from_db(Gws::User, json)
    end
    raise "404" unless @after_item

    if @after_item.id
      @before_item = Gws::User.site(@cur_site).find(@after_item.id) rescue nil
    end
  end
end
