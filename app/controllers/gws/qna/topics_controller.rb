class Gws::Qna::TopicsController < ApplicationController
  ...
  def set_crumbs
    ...
    if @category.present?
      ...
      @crumbs << [t("modules.gws/qna"), action: :index]
    end
  end

  def set_category
    @categories = Gws::Qna::Category.site(@cur_site).readable(@cur_user, @cur_site).tree_sort
    if category_id = params[:category].presence
      @category ||= Gws::Qna::Category.site(@cur_site).readable(@cur_user, @cur_site).where(id: category_id).first
    end
  end
