require "net/imap"
class Webmail::GroupMailsController < ApplicationController
  ...
  def set_crumbs
    ...
    @crumbs << [t("webmail.mail"), webmail_group_mails_path]
  ...
  def reply_all
    ...
    group = SS::Group.find(params[:group])

    @item.to = @item.to.reject do |c|
      c.include?(group.imap_setting.imap_account)
    end
