class Gws::Workflow2::RouteOption::OccupationComponent < ApplicationComponent
  ...
  def items
    @items ||= Gws::UserOccupation.site(cur_site).only(:id, :name, :updated)
  end
  ...
  def call
    cache_component do
      tag.optgroup(label: I18n.t("gws/workflow2.options.route_optgroup.user_occupation"), class: "occupation") do
        items.each do |item|
