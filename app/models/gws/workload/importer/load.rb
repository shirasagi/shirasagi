module Gws::Workload::Importer
  class Load < Base
    ...
    def headers
      %w(id name year coefficient order graph_state color).map { |head| model.t(head) }
    end
    ...
    def update_row(row, index)
      ...
      row_coefficient = row[model.t("coefficient")].to_s.strip
      ...
      item.name = row_name
      item.year = row_year
      item.coefficient = row_coefficient
