class Gws::Workflow2::Route
  ...
  permit_params i18n_name_translations: I18n.available_locales
  ...
  class << self
    def new_from_route(source_route)
      ...
      I18n.available_locales.each do |lang|
        if source_route.i18n_name_translations[lang].present?
          i18n_name[lang] = "[#{I18n.t("workflow.cloned_name_prefix")}] #{source_route.i18n_name_translations[lang]}"
        end
      end
      ret.i18n_name_translations = i18n_name
    ...
    def route_options(user, cur_site:, item: nil, public_only: false, selected: nil)
      ...
      routes.only(:id, :i18n_name).each do |route|
        ret << [ route.i18n_name, route.id.to_s ]
      end
      ...
      if selected && BSON::ObjectId.legal?(selected) && ret.none? { |_name, route_id| route_id == selected }
        ...
        if selected_route
          ret << [ selected_route.i18n_name, selected ]
        end
      end
    ...
    def search_name(params)
      ...
      all.keyword_in params[:name], *search_fields
    end
  ...
  def validate_i18n_name
    ...
    I18n.available_locales.each do |locale|
      ...
      if local_name.length > MAX_NAME_LENGTH
        errors.add :i18n_name, :too_long, count: MAX_NAME_LENGTH
      end
    end
  end
