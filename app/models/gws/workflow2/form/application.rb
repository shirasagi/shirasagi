class Gws::Workflow2::Form::Application < Gws::Workflow2::Form::Base
  ...
  def new_file_name_translations
    timestamp = Time.zone.now.strftime("%Y%m%d")
    seq = next_style_sequence!
    ret = {}.with_indifferent_access
    SS.locales_in_order.each do |lang|
      loocal_name = i18n_name_translations[lang].presence || i18n_name_translations[I18n.default_locale]
      ret[lang] = [ loocal_name, timestamp, seq ].compact.join("_")
    end
    ret
  end
end
