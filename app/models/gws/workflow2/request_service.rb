class Gws::Workflow2::RequestService
  ...
  def call
    if item.form_id && item.form.blank?
      item.errors.add :base, :unable_to_request_due_to_deleted_form
      return false
    end
    if item.form.closed?
      item.errors.add :base, :unable_to_request_due_to_closed_form
      return false
    end
    ...
    item.workflow_approver_target = workflow_approver_target
