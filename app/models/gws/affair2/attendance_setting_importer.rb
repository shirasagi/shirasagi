class Gws::Affair2::AttendanceSettingImporter
  ...
  def import
    ...
    SS::Csv.foreach_row(in_file, headers: true) do |row, idx|
      ...
      organization_uid = row[model.t(:organization_uid)].to_s.strip
      start_date = row[model.t(:start_date)].to_s.strip
      close_date = row[model.t(:close_date)].to_s.strip
      leave_setting_name = row[model.t(:leave_setting_id)].to_s.strip
      duty_setting_name = row[model.t(:duty_setting_id)].to_s.strip

      # user and settings
      leave_setting = Gws::Affair2::LeaveSetting.site(site).where(name: leave_setting_name).first rescue nil
      duty_setting = Gws::Affair2::DutySetting.site(site).where(name: duty_setting_name).first rescue nil

      item.cur_site = site
      item.cur_user = user
      item.organization_uid = organization_uid
      item.duty_setting = duty_setting
      item.leave_setting = leave_setting
