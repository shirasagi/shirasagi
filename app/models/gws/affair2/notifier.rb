class Gws::Affair2::Notifier
  ...
  def deliver_record_entered(to_users, opts = {})
    @to_users = to_users
    @send_date = Time.zone.now
    ...
    @from_user = item.workflow_user
    @subject = I18n.t("gws_notification.#{@model}.record_entered", name: item.name)
  ...
  def deliver_record_confirmed(to_users, opts = {})
    @to_users = to_users
    @send_date = Time.zone.now
    ...
    @from_user = item.workflow_user
    @subject = I18n.t("gws_notification.#{@model}.record_confirmed", name: item.name)

    @body = []
    @body << "結果時間が確認済みになりました。"
    @body << ""
    @body << "- タイトル"
    @body << "  #{@item.name}"
    @body << ""
