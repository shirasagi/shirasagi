module Workflow::Approver
  ...
  attr_accessor :workflow_reset, :workflow_cancel_request,
    :workflow_approver_target, :workflow_approver_alternate
  ...
  included do
    ...
    permit_params :workflow_reset, :workflow_cancel_request, :workflow_approver_target, :workflow_approver_alternate
  ...
  def set_workflow_approver_target
    return unless workflow_approver_target.present?
    return unless user = Gws::User.site(site).find(workflow_approver_target) rescue nil

    copy = workflow_approvers.to_a
    copy[0][:user_id] = user.id
    copy[0][:user_type] = 'superior'
    self.workflow_approvers = Workflow::Extensions::WorkflowApprovers.new(copy)
  end
