module Cms::Model::Node
  ...
  def validate_ancestors
    ...
    p = parent
    while p
      break errors.add(:base, :routed_folders_under_uploader) if p.route.start_with?('uploader/')
      p = p.parent
    end
  end
  ...
  def destroy_children
    %w(nodes pages parts layouts).each { |name| send(name).destroy_all }
  end
