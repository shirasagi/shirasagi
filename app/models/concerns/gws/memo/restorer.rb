module Gws::Memo::Restorer
  ...
  def set_to_member_ids_if_blank
    if self.to_member_ids.blank?
      self.to_member_ids = [@cur_user.id]
    end
  ...
  def set_deleted
    member_ids.each do |id|
      self.deleted[id.to_s] = @datetime if id != @cur_user.id
    end
    self.deleted["sent"] = @datetime unless @sent_by_cur_user
  end
  ...
  def member_ids
    (self.to_member_ids + self.cc_member_ids + self.bcc_member_ids).uniq
  end
end
