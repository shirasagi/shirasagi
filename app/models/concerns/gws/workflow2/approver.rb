module Gws::Workflow2::Approver
  ...
  included do
    ...
    if respond_to?(:liquidize)
      liquidize do
        ...
        export :workflow_agent
        export :workflow_state
        export as: :workflow_state_label do
          if workflow_state.present?
            I18n.t("workflow.state.#{workflow_state}")
          end
        end
        export :workflow_comment
        export :workflow_current_level
        export as: :workflow_approved do
          approved
        end
        export as: :workflow_requested do
          requested
        end
      end
    end
  ...
  # Gws::Workflow::File には state はない（かつて存在していたが、存在していると逆に邪魔・不具合の温床となりそうなので削除）。
  ...
  def update_workflow_user(site, user, group)
    if user.blank?
  ...
  def update_workflow_agent(site, user, group)
    if user.blank?
