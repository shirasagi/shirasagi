...
SS.locales_in_order.each do |lang|
  ...
  json.set! "i18n_name_#{lang}", translations[lang]
end
...
item.form.try do |form|
  json.form do
    ...
    translations = item.form.i18n_name_translations
    json.name translations[I18n.default_locale]
    SS.locales_in_order.each do |lang|
      next if translations[lang].blank?
      json.set! "name_#{lang}", translations[lang]
    end
  end
  ...
  json.column_values do
    json.array! item.column_values.order_by(order: 1, name: 1) do |column_value|
      if value = column_value.send(:value)
        if column_value.class == Gws::Column::Value::TextArea
