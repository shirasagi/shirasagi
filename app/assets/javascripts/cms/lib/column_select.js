globalThis.Cms_Column_Select = (function () {
  function Cms_Column_Select() {
  }
  ...
  Cms_Column_ChildSelect.prototype.render = function () {
    ...
    this.$container.on("change", function (ev) {
      self.maybeChanged(ev.target)
    });
    $(document).on("ss:columnAdded", function (ev, el) {
      self.maybeAdded(el);
    });
    $(document).on("ss:columnDeleted", function () {
      self.reload();
    });
    self.reload();
  };
