globalThis.Cms_ConditionForms = (function () {
  ...
  Cms_ConditionForms.prototype.render = function () {
    ...
    self.$el.on("change", ".ajax-selected", function () {
      self.reloadColumnList();
    });
    self.$el.on("click", ".filter-table .deselect", function (ev) {
      return self.onDeleteFilter(ev);
    });
    self.$el.on("click", ".add-filter-btn", function (ev) {
      return self.onAddFilter(ev);
    });
  };
