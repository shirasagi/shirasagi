globalThis.SS_Workflow = (function () {
  function SS_Workflow(el, options) {
    this.$el = $(el);
    ...
    this.tempFile = new SS_Addon_TempFile(
      ".mod-workflow-approve .upload-drop-area", this.options.user_id,
      {
        select: function (files, dropArea) {
          pThis.onDropFile(files, dropArea);
        }
      }
    );
...
globalThis.SS_WorkflowRerouteBox = (function () {
  function SS_WorkflowRerouteBox(el, options) {
    this.$el = $(el);
    this.options = options;
  ...
  return SS_WorkflowRerouteBox;
})();

globalThis.SS_WorkflowApprover = (function () {
  function SS_WorkflowApprover(options) {
    this.options = options;
    this.render();
  };
