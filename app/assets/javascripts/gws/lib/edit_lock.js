this.Gws_EditLock = (function () {
  ...
  function Gws_EditLock(selector, lock_url, unlock_url) {
    //must return void
  ...
  Gws_EditLock.prototype.updateView = function (lock_until) {
  ...
  Gws_EditLock.prototype.refreshLock = function () {
    if (this.unloading) {
      return;
    }
    $.ajax({
  ...
  Gws_EditLock.prototype.releaseLock = function () {
    this.unloading = true;
    $.ajax({
      type: "POST",
      url: this.unlock_url,
  ...
  Gws_EditLock.prototype.releaseLockOnCancel = function () {
    this.releaseLock();
    return true;
  };
