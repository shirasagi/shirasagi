...
globalThis.Gws_Schedule_Csv = (function () {
  ...
  Gws_Schedule_Csv.prototype.renderResult = function (data) {
    ...
    if (data.items) {
      ...
      if (count.exist) tabs += ejs.render('<span class="ml-2 import-exist"><%= text %>(<%= count.exist %>)</span>', {
        text: i18next.t('gws/schedule.import.exist'),
        count: count
      });
      if (count.entry) tabs += ejs.render('<span class="ml-2 import-entry"><%= text %>(<%= count.entry %>)</span>', {
        text: i18next.t('gws/schedule.import.entry'),
        count: count
      });
      if (count.saved) tabs += ejs.render('<span class="ml-2 import-saved"><%= text %>(<%= count.saved %>)</span>', {
        text: i18next.t('gws/schedule.import.saved'),
        count: count
      });
      if (count.error) tabs += ejs.render('<span class="ml-2 import-error"><%= text %>(<%= count.error %>)</span>', {
        text: i18next.t('gws/schedule.import.error'),
        count: count
      });
      tabs += '</div>'
