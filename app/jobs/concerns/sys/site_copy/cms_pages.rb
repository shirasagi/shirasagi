module Sys::SiteCopy::CmsPages
  ...
  def copy_cms_page(src_page)
    Rails.logger.info("♦︎ [copy_cms_page] コピー開始: #{src_page.filename}(#{src_page.id}), route: #{src_page.route}")
  ...
  def copy_cms_pages
    ...
    Rails.logger.info("♦︎ [copy_cms_pages] コピー対象ページ数: #{page_ids.size}")
  ...
  def resolve_page_reference(id)
    cache(:pages, id) do
      ...
      if src_page.blank?
        Rails.logger.warn("♦︎ [resolve_page_reference] #{id}: 参照されているページが存在しません。")
        return nil
      end
