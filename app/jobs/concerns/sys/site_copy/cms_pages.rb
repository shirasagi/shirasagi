module Sys::SiteCopy::CmsPages
  ...
  def copy_cms_page(src_page)
    Rails.logger.info{ "♦[copy_cms_page] コピー開始: #{src_page.filename}(#{src_page.id}), route: #{src_page.route}" }
  ...
  def copy_cms_pages
    ...
    Rails.logger.info{ "♦[copy_cms_pages] コピー対象ページ数: #{page_ids.size}" }
    page_ids.each do |page_id|
      page = Cms::Page.site(@src_site).find(page_id) rescue nil
      next if page.blank?
      Rails.logger.info{ "♦[copy_cms_pages] ページコピー開始: #{page.filename} (#{page.id})" }
      copy_cms_page(page)
      Rails.logger.info{ "♦[copy_cms_pages] ページコピー終了: #{page.filename} (#{page.id})" }
    end
  end
  def resolve_page_reference(id)
    cache(:pages, id) do
      ...
      Rails.logger.info{ "♦[resolve_page_reference] 参照ページコピー開始: #{src_page.filename} (#{src_page.id})" }
