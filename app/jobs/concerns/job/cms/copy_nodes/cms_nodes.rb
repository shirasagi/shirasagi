module Job::Cms::CopyNodes::CmsNodes
  ...
  def copy_inquiry_second_columns(src_node, dest_node)
    InquirySecond::Column.where(site_id: src_node.site_id, node_id: src_node.id).order_by(updated: 1).each do |src_inquiry_column|
      @task.log("#{src_inquiry_column.name}(#{src_inquiry_column.id}): InquirySecond::Column をコピーします。")
      dest_inquiry_column = InquirySecond::Column.new src_inquiry_column.
        attributes.except("id", "_id", "node_id", "site_id", "created", "updated")

      dest_inquiry_column.site_id = dest_node.site_id
      dest_inquiry_column.node_id = dest_node.id
      dest_inquiry_column.save!
      @task.log("#{src_inquiry_column.name}(#{src_inquiry_column.id}): InquirySecond::Column をコピーしました。")
    end
  end
