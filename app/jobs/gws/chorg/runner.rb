class Gws::Chorg::Runner < Gws::ApplicationJob
  ...
  def perform(name, adds_group_to_site)
    ...
    @item = Gws::Chorg::Revision.site(@cur_site).find_by(name: name)
    @item = Gws::Chorg::Revision.acquire_lock(@item, 1.hour.from_now)
    unless @item
      put_log("already running")
      return
    end

    Gws::Chorg::Revision.ensure_release_lock(@item) do
      init_context
