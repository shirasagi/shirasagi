class Gws::Elasticsearch::Indexer::BoardTopicJob < Gws::ApplicationJob
  ...
  def enum_es_docs
    Enumerator.new do |y|
      topic_criteria = Gws::Board::Topic.site(site).topic.without_deleted
      ...
      if @original_id == :all
        ...
        post_criteria = Gws::Board::Post.site(site).without_deleted
        post_criteria = post_criteria.in(topic_id: all_topic_ids)
        each_item(criteria: post_criteria) do |post|
          puts post.name
          topic = Gws::Board::Topic.find(post.topic_id)
