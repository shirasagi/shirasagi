...
create_bookmark(cur_user: u('sys'), name: '企画政策課座席表', url: "/.g#{@site.id}/share/folder-#{@sh_files[0].folder_id}/files/#{@sh_files[0].id}", bookmark_model: 'share')
create_bookmark(cur_user: u('sys'), name: 'SHIRASAGI公式サイト', url: 'http://www.ss-proj.org/', bookmark_model: 'other')
create_bookmark(cur_user: u('sys'), name: 'お気に入り', url: "/.g#{@site.id}gws/bookmarks#{@sh_files[0].folder_id}/files/#{@sh_files[0].id}", bookmark_model: 'bookmark')
