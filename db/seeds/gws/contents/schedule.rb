...
def third_sunday_of_month(base_date)
  first_sunday_of_month(base_date) + 14.days
end
...
create_schedule_plan(name: "地域イベント打ち合わせ", member_ids: [ u("sys").id, u("admin").id, u("user1").id, u('user2').id ],
  start_at: first_friday_of_month(base_date).strftime('%Y-%m-%d 10:00'),
  end_at: first_friday_of_month(base_date).strftime('%Y-%m-%d 11:00'), repeat_type: 'weekly', interval: 1, wdays: [],
...
@sch_plan1 = create_schedule_plan(
  ...
  repeat_end: first_tuesday_of_month(base_date) + 5.month,
...
@sch_plan1 = create_schedule_plan(
  ...
  allday: "allday",
  start_on: third_thursday_of_month(base_date + 1.months).strftime('%Y-%m-%d'),
  end_on: (third_thursday_of_month(base_date + 1.months) + 1.day).strftime('%Y-%m-%d'),
...
create_schedule_plan(
  cur_user: u("user4"), name: "有給休暇(#{first_tuesday_of_month(base_date + 1.month).strftime('%m%d')})", member_ids: [u("user4").id],
  allday: "allday",
...
create_schedule_plan(
  cur_user: u("user5"), name: "有給休暇(#{first_tuesday_of_month(base_date + 1.month).strftime('%m%d')})", member_ids: [u("user5").id],
  allday: "allday",
...
create_schedule_plan(
  cur_user: u("user2"), name: "有給休暇(#{third_thursday_of_month(base_date + 1.month).strftime('%m%d')})", member_ids: [u("user2").id],
  allday: "allday",
...
create_schedule_plan(name: "企画セミナー", cur_user: u("sys"), member_ids: [ u("sys").id],
...
create_schedule_plan(name: "企画勉強会", member_ids: [u("user1").id],
...
create_schedule_plan(name: "アカサギ商事様来庁", member_ids: [ u("user2").id, u("user4").id],
...
create_schedule_plan(name: "アオサギ支所訪問", member_ids: [u("user4").id],
...
create_schedule_plan(name: "クロサギ商事様来庁", member_ids: [u("user4").id], cur_user: u("user4"),
...
create_schedule_plan(name: "清掃活動", cur_user: u("user2"), member_ids: [u('user2').id, u("user4").id],
