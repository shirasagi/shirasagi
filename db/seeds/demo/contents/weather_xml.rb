...
weather_xml_node = save_node route: "rss/weather_xml", filename: "weather",
  name: "気象庁防災XML", layout_id: @layouts["general"].id, rss_max_docs: 100, page_state: "closed",
  earthquake_intensity: "5+", anpi_mail_id: @ezine_anpi.try(:id),
...
action3 = Jmaxml::Action::SwitchUrgency.site(@site).where(name: '緊急災害：トップページ').first_or_create(
  urgency_layout_id: @layouts["urgency-layout/top-level1"].id)
