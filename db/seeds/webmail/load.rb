...
create_address(u('sys'), address_group_id: @address_group[1].id, name: "サイト管理者", kana: "サイトカンリシャ",
...
# )
create_address(u('sys'), address_group_id: @address_group[0].id, name: "伊藤　幸子", kana: "イトウ　サチコ",
               email: 'user4@demo.ss-proj.org'
)
create_address(u('sys'), address_group_id: @address_group[1].id, name: "斉藤　拓也", kana: "サイトウ　タクヤ",
               email: 'user3@demo.ss-proj.org'
)
create_address(u('sys'), address_group_id: @address_group[0].id, name: "渡辺　和子", kana: "ワタナベ　カズコ",
               email: 'user２@demo.ss-proj.org'
)
create_address(u('sys'), address_group_id: @address_group[1].id, name: "鈴木　茂", kana: "スズキ　シゲル",
               email: 'user1@demo.ss-proj.org'
)
create_address(u('sys'), address_group_id: @address_group[1].id, name: "高橋　清", kana: "タカハシ　キヨシ",
               email: 'user5@demo.ss-proj.org'
)
create_address(u('sys'), address_group_id: @address_group[0].id, name: "黒鷺　晋三", kana: "クロサギ　シンゾウ",
               email: 'kurosagi@example.jp' , company: '株式会社シラサギ', tel: '080-0000-0001'
)
create_address(u('sys'), address_group_id: @address_group[0].id, name: "白鷺　次郎", kana: "シロサギ　ジロウ",
...
puts "#Create filters"
 ...
 def create_filters(user, deta)
   create_item(Webmail::Filter, user, deta)
 end
@create_filtes = [
  create_filters(u("sys"), name: '広報シラサギ', state: 'enabled', order: '10',
                 conditions: [{ field: "subject", operator: "include", value: "広報シラサギ" },
